...
<role_granted> ::=
  !! Vgl. <<fblangref30-security-grant-rolesyntax,`GRANT`-Syntax>> !!
...
<grantee_list> ::=
  !! Vgl. <<fblangref30-security-grant-privsyntax,`GRANT`-Syntax>> !!
----
...
Wenn der aktuelle Benutzer mit vollen <<fblangref30-security-administrators,administrator>> Berechtigungen in der Datenbank angemeldet ist, entfernt `REVOKE ALL ON ALL` alle Berechtigungen, egal wer sie gewährt hat.
