...
When moving a database between different Firebird installations -- without using backup and restore -- and the destination Firebird installation uses a different ICU version, querying on fields with those collations or using those indices will fail.
...
* Added word-joiner to commandline options in text to prevent linebreaks inside options.
* Misc. copy-editing
|===
