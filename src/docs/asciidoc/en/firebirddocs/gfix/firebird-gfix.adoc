...
.gfix usage instructions for Firebird 5.0.0
----
...
plausible options are:
   ...
   -par(allel)          parallel workers <n> (-sweep, -icu)
   -pa(ssword)          default password
   -repl(ica)           replica mode <none / read_only / read_write>
...
Accepts the username of the SYSDBA user, or the owner of the database to be specified.
...
You will notice that some commands do not give any printed output at all.
...
[NOTE]
====
When logging into a database on a remote server, you will always be required to pass the `-{wj}user` and `-{wj}pa[ssword]`, or `-tru[sted]` parameters.
====
...
Before Firebird 3, the only option was to back up and restore the database.
