...
If you have not defined the above environment variables, some commands will not work unless you supply `-{wj}user` and `-{wj}pa[ssword]` on the command line.
For example:
...
[NOTE]
====
When logging into a database on a remote server, you will always be required to pass the `-{wj}user` and `-{wj}pa[ssword]` parameters.
====
