...
Available switches:
    <<gstat-cmdline-all,-a>>      analyze data and index pages
    <<gstat-cmdline-data,-d>>      analyze data pages
    <<gstat-cmdline-encryption,-e>>      analyze database encryption
    <<gstat-cmdline-header,-h>>      analyze header page ONLY
    <<gstat-cmdline-index,-i>>      analyze index leaf pages
    <<gstat-cmdline-system,-s>>      analyze system relations in addition to user tables
    <<gstat-cmdline-username,-u>>      username
    <<gstat-cmdline-password,-p>>      password
    <<gstat-cmdline-fetch-password,-fetch>>  fetch password from file
    <<gstat-cmdline-record,-r>>      analyze average record and version length
    <<gstat-cmdline-table,-t>>      tablename <tablename2...> (case sensitive)
    <<gstat-cmdline-role,-role>>   SQL role name
    <<gstat-cmdline-trusted,-tr>>     use trusted authentication
    <<gstat-cmdline-z,-z>>      display version number
...
[listing,subs=+quotes]
----
-T[ABLE] _table_name_ [ -T[ABLE] _table-name_ ]...
----

On Firebird 2.1 and older, you can only specify `-{wj}t[able]` *once*.

.Alternative syntax when placed *after* the database name
[listing,subs=+quotes]
----
-T[ABLE] _table_name_ [ __table_name__... ]
----

If you want to specify multiple tables, repeat the `-{wj}t[able]` switch (requires Firebird 2.5 or higher _gstat_).
Or if you need to use a single `-{wj}t[able]` switch (e.g. on Firebird 2.1 and older), it must be specified _after_ the database name.
...
The table names must match exactly as stored in the metadata.
...
Note that specifying table names in this manner also analyses all indices associated with those tables, unless only <<gstat-cmdline-data>> or <<gstat-cmdline-index>> is specified.
...
The <<gstat-cmdline-table>> switch can receive a list of table names.
...
For this reason, it is better to specify `-{wj}t` as a repeated option;
this requires Firebird 2.5 or higher:

----
> gstat -u sysdba -t EMPLOYEE -t JOB employee
...
Alternatively, or if you're using Firebird 2.1 or older, call `gstat` with the database name as the very _first_ parameter, or at least put the `-{wj}t[able]` option _after_ the database name:
...
|17 Oct 2025
|MR
|Documented that `-{wj}t[able]` switch can be repeated (since Firebird 2.5)
