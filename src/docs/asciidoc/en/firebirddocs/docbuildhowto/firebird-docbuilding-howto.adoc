...
Several Java tools are used to produce the HTML and PDF docs from the AsciiDoc and DocBook XML source.
...
[NOTE]
====
We have migrated most of the DocBook documentation sources to AsciiDoc.
However, some older documentation -- like release notes for older versions -- will not be migrated.

If you feel the need to modify and build DocBook documentation, it might be an indication that they do need to be migrated to AsciiDoc.
...
[NOTE]
====
* If you build the DocBook PDF target, you might receive tons of error messages.
...
NOTE: This example uses `asciidocPdf` as task name, but release notes of Firebird 2.5 and older are in DocBook.
...
When in doubt, ask for help on https://groups.google.com/g/firebird-devel[firebird-devel^].
