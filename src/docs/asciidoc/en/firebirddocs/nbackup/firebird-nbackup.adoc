...
Copyright (C) 2005-2026 Firebird Project and all contributing authors, under the https://firebirdsql.org/docs/html/en/licenses/pdl/public-documentation-license.html[Public Documentation License Version 1.0].
...
_Nbackup_ is a physical backup utility included with Firebird.
It offers possibilities -- like incremental backup -- not present in _gbak_ -- Firebird's other backup tool.
Both programs have their strengths and weaknesses, and are complementary.
...
. Making and restoring of -- both full and _incremental_ -- _physical_ backups.
+
An _incremental backup_ only contains the mutations since some previous backup.
+
A _physical backup_ stores the data pages as-is.
. Locking the main database file, so content of the main database are stable until it's unlocked.
+
You can subsequently back it up yourself with file copy or backup tools of your own choice.
+
In this mode, nbackup doesn't back up anything;
it just creates the conditions so you can safely make the backup yourself.
There's a provision for restoring here, too. 
Locking is necessary to make valid backups of Firebird databases inside virtual machines: without locking a database before backup or snapshot of a VM, the database file in the VM backup could be inconsistent.
...
Both modes::
High speed (as high as hardware and OS will allow), because nbackup doesn't look at the actual data.
In backup mode, the contents are written more or less blindly to the backup file.

Backup/restore mode::
Time and disk space savings, because you don't need to make a full backup every time.
This can make a huge difference with databases in the gigabyte range.

Lock/unlock mode::
Total freedom in your choice of backup, copy, and/or compression tools.

[[nbackup-limitations]]
=== Limitations of nbackup

* Nbackup will not sweep the database during backup;
it only copies physical data pages without interpreting or rewriting data on those pages.
* Nbackup does not compact the database during restore;
it only writes physical data pages as they appear in the backup file.
...
** A backup cannot be restored on an incompatible platform,
** or under another server version (it cannot perform an ODS upgrade).
* Nbackup should not be used on multi-file databases.
...
|-{wj}BACKUP {_level_ {vbar} _guid_} _database_ [_filename_]
|-{wj}B
a|Make backup of database to file. +
First argument is either the backup _level_, or -- Firebird 4{plus} -- the _guid_ of the previous backup.
|{nbsp}

|-{wj}RESTORE _database_ [_filename_ ...]
|-{wj}R
...
To unlock, use `-{wj}UNLOCK`.
|{nbsp}

|-{wj}UNLOCK _database_
|-{wj}UN
|Unlock database and merge back changes.
|3.0

|-{wj}N _database_
|{nbsp}
|Unlock database and merge back changes (_deprecated_ since 3.0, use `-{wj}UNLOCK`).
|{nbsp}

|-{wj}FIXUP _database_
|-{wj}F
|Forcibly unlock user-restored database.

Changes are *not* merged back.
|{nbsp}

|-{wj}SIZE
|-{wj}S
|Report database size in database pages (with `-{wj}LOCK`)
|2.1
...
|-{wj}PASSWORD _password_
|-{wj}P
|Supply password (with `-{wj}BACKUP`, `-{wj}LOCK`, `-{wj}UNLOCK`)
|{nbsp}

|-{wj}FETCH_PASSWORD _filename_
|-{wj}FE
...
|-{wj}FIXUP
|Forcibly unlock after user restore
|file
|===
...
Access to the backup file is always on the filesystem level.

[[nbackup-backups]]
== Making and restoring backups

To begin with: `nbackup` is located in your Firebird folder.
...
For instance, assuming the database is located in  `C:\Data`, and `nbackup.exe` is in the search path of Windows:
...
Or, if Firebird (from version 2.5) is running on a non-standard port, in this example, 3051:
[listing]
----
...
On Firebird 3.0 and higher, specifying localhost may be required if you use SuperServer mode.
Otherwise, nbackup may attempt to use an embedded connection to lock the database, and this can fail because in SuperServer mode, the server opens the database file with exclusive access.

In Firebird 3.0 and higher, in a case of successful completing the backup, nbackup prints short statistics:
----
time elapsed    0 sec
page reads      307
page writes     307
...
* The _backup level_ 0 indicates a full backup.
Backup levels greater than 0, or a GUID, are used for incremental backups;
...
* [[nbackup-backups-nologin]]The `-{wj}USER` and `-{wj}PASSWORD` parameters may be omitted if at least one of the following conditions is met:
+
--
...
--
+
For clarity and brevity, the `-{wj}USER` and `-{wj}PASSWORD` parameters are not used in the examples.
* Starting with Firebird 2.5, instead of  `-PASSWORD __password__`, you may also use `-FETCH_PASSWORD __filename__`.
This will cause nbackup to fetch the password from the given file.
With `-{wj}FETCH_PASSWORD`, the password itself doesn't appear in the command and will thus be better shielded against people who might otherwise pick it up via the command history, the `w` command on Unix or from a script or batch-file.
* Firing of database triggers can be disabled by:
+
--
** In Firebird 3.0 and up, specifying the `-{wj}NODBTRIGGERS` option.
** In Firebird 2.1 and up, specifying the `-{wj}T` option.
+
The `-{wj}T` option was deprecated in Firebird 3.0 in favour of `-{wj}NODBTRIGGERS`.
--
For more information, see <<nbackup-backups-dbtriggers,[ref]_Suppressing database triggers_>>.
* Starting with Firebird 2.1.4, it is possible to force direct I/O on or off by specifying `-{wj}D ON` or `-{wj}D OFF`.
For details and background see <<nbackup-backups-directio,[ref]_Direct I/O_>>, elsewhere in this manual.
* The different options (`-{wj}BACKUP`, `-{wj}USER` etc.) may occur in any order.
Of course each parameter should be immediately followed by its own argument(s).
In the case of `-{wj}BACKUP` there are three of them: backup level or guid, database, and backup file -- in that order!
* If the `-{wj}BACKUP` parameter comes last, you _may_ leave out the name of the backup file.
...
. The main database file is locked by changing an internal state flag.
+
From this moment on, mutations in the database are written to a temporary file -- the difference file or _delta file_.
...
.. The contents of the delta file are integrated with the main database file.
.. The database is unlocked (flag goes back to "`normal`") and the delta is removed.
...
* When restoring, the `-{wj}RESTORE` parameter _must_ come last, for reasons that will become clear later.
* Instead of a database filename you may also use an alias.
* If the specified database file already exists, the restore fails, and you get an error message.
* You may omit the name of the backup file.
...
Any credentials supplied via the `-{wj}USER` and `-{wj}PASSWORD` parameters are ignored.
The same goes for passwords read from a file.
However, nbackup _does_ try to read the password from the file if the `-{wj}FETCH_PASSWORD` parameter is present, and if an error occurs, the entire operation is abandoned.
...
An incremental backup of level `__N__` always contains the database mutations since the most recent level `__N__-1` backup.

If instead of a level, a GUID is specified, all changes since the backup identified by the GUID are backed up.
...
* Because it is not known beforehand how many filenames will follow the `-{wj}RESTORE` switch (as we don't specify a level when restoring), nbackup considers all arguments after the `-{wj}RESTORE` to be names of backup files.
It is for this reason that no other options may follow the list of filenames.
* There is no formal limit to the number of backup levels, but we think it will rarely make sense to go beyond 3 or 4.
...
The question of where the <<nbackup-backups-workings,delta>> has to be placed for a database on a raw device was initially overlooked.
On Posix systems, if the database was located at e.g. `/dev/hdb5`, the default delta would be created as `/dev/hdb5.delta`.
...
These triggers can fire upon opening or closing a database connection.
As part of the backup process, nbackup opens a regular connection to the database (in some versions even more than once).
To prevent database triggers from firing inadvertently, the `-{wj}T` or -- since Firebird 3.0 -- `-{wj}NODBTRIGGERS` switch can be used.

[[nbackup-backups-directio]]
=== Direct I/O (Firebird 2.1.4+)

Originally, nbackup used direct I/O only when making a backup under Windows.
...
In 2.1.4 and 2.5 the original behaviour was restored, but this time as a default that was overridable by a newly added parameter: `-{wj}D`.
...
Under Windows, this is realized by setting `FILE_FLAG_NO_BUFFERING`.
...
Even if the user specified `-{wj}D on` explicitly, this doesn't lead to a warning or error message.

[[nbackup-backups-inform-options]]
=== Informational options (Firebird 2.5+)

Since Firebird 2.5, nbackup also offers the following informational options:

[horizontal.compact]
`-{wj}Z`::
Shows single-line version information.
+
This option can be used independently, but also in combination with other parameters, such as `-{wj}BACKUP`, `-{wj}RESTORE`, `-{wj}LOCK`, etc.

`-{wj}?`::
Shows a summary of nbackup's usage and command-line parameters.
+
If this option is present, all other options are ignored!
...
For this, the program `fbsvcmgr` on the local machine can be used;
it is located in the same folder as `nbackup` and the other Firebird command-line tools.
...
* The Services Manager always requires authentication, be it automatic (root under Posix, trusted under Windows) or explicit through the parameters `-{wj}user` and `-{wj}password`.
...
* Restoring (`-{wj}action_nrest`) also requires authentication, but once verified the credentials are not used in any way.
...
* Specifying `-{wj}nbk_no_triggers` or `-{wj}nbk_direct` with `-{wj}action_nrest` leads to an error message.
Nbackup itself is more lenient here: it simply ignores the `-{wj}T` and `-{wj}D` parameters if they are used in the wrong context.
* Instead of a database filename you may also use an alias.
...
Here, "`Locking`" means that the main database file is frozen temporarily, not that no changes can be made to the database.
Just like in backup mode, mutations are directed to a temporary delta file;
upon unlocking, the delta file is merged into the main file.

As a reminder: `nbackup` is located in your Firebird folder.
Typical locations are e.g. `C:\Program Files\Firebird\Firebird_3_0` or `C:\Program Files\Firebird\Firebird_2_0\bin` (Windows) or `/opt/firebird/bin` (Linux).
There's no GUI;
you launch it from the command prompt or call it from within a batch file or application.
...
. Lock the database with the `-{wj}LOCK` switch:
+
[listing,subs=+quotes]
----
nbackup [-U _user_ -P _password_] -LOCK _database_
----
...
. Unlock the database with `-UNLOCK`, or -- Firebird 2.5 and older -- `-N` (uNlock):
+
[listing,subs=+quotes]
----
nbackup [-U _user_ -P _password_] -UNLOCK _database_
----

The last command will also cause any mutations -- written to the delta file -- to be merged into the main file.
...
* The `-{wj}USER` and `-{wj}PASSWORD` parameters may be omitted if the environment variables `ISC_USER` and `ISC_PASSWORD` are set, if you are root on a Posix system, or if trusted authentication under Windows permits it.
For a detailed description see the <<nbackup-backups-nologin,comments under [ref]_Making full backups_>>.
* Starting with Firebird 2.5, instead of `-{wj}PASSWORD __password__` you may also use `-{wj}FETCH_PASSWORD __filename__`.
* Both `-{wj}LOCK` and `-{wj}UNLOCK` (`-N` in Firebird 2.5 and older) make a regular connection to the database, so in Firebird 2.1 and above it may be wise to add the `-{wj}NODBTRIGGERS` (`-T` in Firebird 2.5 and older) options (see <<nbackup-backups-dbtriggers,[ref]_Suppressing database triggers_>>).
* If you're locking a raw-device database with Firebird 2.1 or above, the `-{wj}SIZE` option can be very helpful for determining the actual database size;
see <<nbackup-lock-unlock-rawdevices>>.
* You can optionally add `-{wj}Z` to have version information printed on the first line of the output.

[WARNING]
====
What goes for backup/restore also applies to the lock/unlock switches: do not use them on multi-file databases.
Don't let nbackup loose on multi-file databases at all!
====

[[nbackup-restore-and-fixup]]
=== Restoring a backup made after "`nbackup -{wj}LOCK`"

A copy of a locked database is itself a locked database too, so you can't just copy it back and start using it.
Should your original database get lost or damaged, and the self-made copy needs to be restored, or you wish to use the copy on another machine, proceed like this:

. Copy/restore/unzip the backed-up database file yourself with the necessary tools.
. Now forcibly unlock the database, with `-{wj}FIXUP`:
+
[listing,subs=+quotes]
----
nbackup -FIXUP _database_
----
+
Here too, you can optionally use an alias instead of a filename, and add `-{wj}Z` for version info.
Other options make no sense.

Why are there two unlock switches, `-{wj}UNLOCK` (`-{wj}N`) and `-{wj}FIXUP`?

* `-{wj}UNLOCK` first merges any changes recorded in the delta file into the main database file.
+
After that, the database goes back into normal read/write mode and the temporary file is deleted.
* `-{wj}FIXUP` only changes the state flag of the user-restored database to "`normal`".
The delta file is *not* merged back.

So you use:

* `-{wj}UNLOCK` after having _made_ a copy or backup yourself.
+
This unlocks/reverses the `-{wj}LOCK` issued earlier and merges back any changes since the database was locked.
* `-{wj}FIXUP` after having _restored_ such a copy or backup.
+
This unlocks the copy, and changes are *not* merged back.

.Incorrect use of `-{wj}FIXUP` can result in data-loss.
[CAUTION]
====
The term fixup is used because it doesn't attempt to merge the delta-file, but makes the minimum changes necessary to make the database file usable again.

If you use `-{wj}FIXUP` on a database that is in active use, all changes since it was locked *are lost*.
====
...
`nbackup -{wj}LOCK` does the following:

. Connect to the database;
. Start a transaction;
. Call `ALTER DATABASE BEGIN BACKUP` (this statement has been discussed in the <<nbackup-backups-workings,extra information on `nbackup -{wj}BACKUP`>>);
. Commit the transaction;
. Disconnect from the database.

`nbackup -{wj}UNLOCK` follows the same steps, but with "```ALTER DATABASE END BACKUP```" in step 3.

`nbackup -{wj}FIXUP` works as follows:
...
. Firebird 4+: unless `-SEQUENCE` is specified, the replication sequence is reset
. The file is closed again.

[NOTE]
====
nbackup `-{wj}FIXUP` operates purely on file level and can therefore also be performed without a Firebird server running.
Any credentials supplied via the `-{wj}USER`, `-{wj}PASSWORD` or `-{wj}FETCH_PASSWORD` options are ignored, just as with `-{wj}RESTORE`.
====
...
To prevent having to copy the entire device just to be on the safe side -- possibly wasting huge amounts of time and space -- Firebird 2.1 has introduced a new parameter for nbackup: `-{wj}SIZE`.

This parameter is only valid in combination with `-{wj}LOCK`.
When it is present, nbackup writes the database size in pages to `stdout` after locking the database.
Because the size is given in pages, it has to be multiplied by the database page size to get the actual number of bytes to be copied.
Or, if you use the `dd` copy utility, you could specify the page size as `(i)bs` and the output of `nbackup -{wj}LOCK -{wj}SIZE` as `count`.

[[nbackup-lock-virtual-machine]]
=== Locking/Unlocking database in case of Virtual Machines Backup

Using Virtual Machines backup tools or snapshots without preparing database for such type of backup can lead to a corrupted (i.e. useless) backup copy.

Firebird server intensively uses its own cache in RAM to speed up operations, and implements complex techniques to ensure database consistency at every given moment.
...
As a result, when the virtual machine backup is done, the database file inside it will have the same state as after a hard reset of the VM, and very often such a copy is not consistent (i.e. corrupted).
The chance of such problem is higher when many active users are changing the database, or if there is an active sweep process.

Such inconsistent backups can occur in any virtualized environment, including public clouds.

To create good Firebird database backups with VM backup tools, it is necessary to lock database files with nbackup before the VM backup, and unlock it after.
Usually VM backup tools support execution of custom pre- and post-backup scripts, where you can lock/unlock Firebird databases.

[[nbackup-deltafile]]
== Setting the delta file

By default, the delta file lives in the same directory as the database itself.
The default filename is the database filename with `.delta` appended.
...
* If you specify a bare filename with `[ADD] DIFFERENCE FILE`, the delta will likely _not_ be created in the same directory as the database, but in the current directory as seen from the server.
...
|RDB$GUID
|GUID of the backup (used to check dependencies between files, and for GUID-based incremental backups)
...
Nbackup performs a physical backup of the database pages by copying pages that have been modified since the last backup of the immediately preceding level, or since the specified backup GUID.
...
|TBD
|MR
a|* Remove mention of incremental backup being broken in 2.1.0 and fixed in 2.1.1.
* Removed warnings about issues in very old and now unsupported Firebird versions
* Misc. copy editing and rephrasing

|1.9
|04 Jul 2025
|MR
|Make more explicit that `-{wj}F`/`-{wj}FIXUP` only unlocks and does not merge the delta.
...
All Rights Reserved.
(Contributor contact(s): mark{nbsp}at{nbsp}lawinegevaar{nbsp}dot{nbsp}nl).
