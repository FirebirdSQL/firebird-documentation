...
. Now forcibly unlock the database, _not_ with the `-{wj}N` (or `-{wj}UN`) switch, but with `-{wj}F` (fixup):
+
[listing,subs=+quotes]
----
...
* `-N`/`-UN` after having _made_ a copy or backup yourself.
+
This unlocks/reverses the `-{wj}L` issued earlier and merges back any changes since the database was locked.
* `-F` after having _restored_ such a copy or backup.
+
This unlocks the copy, and changes are *not* merged back.

.Incorrect use of `-{wj}F`/`-{wj}FIXUP` can result in data-loss.
[CAUTION]
====
...
If you use `-{wj}F`/`-{wj}FIXUP` on a database that is in active use, any and all changes since it was locked *are lost*.
====
