...
. Server
** <- <<wireprotocol-op-accept>> -- Server accepts and reports selected protocol, continue with step 3
** <- <<wireprotocol-op-reject>> -- Server can't fulfill the requested protocol
*** Report error `isc_connect_reject` (`335544421`) or equivalent
*** Close connection
** <- <<wireprotocol-responses-generic>> -- Error or other problem
...
. Server <- <<wireprotocol-responses-generic>>
...
*** Report error
*** Close connection (<<wireprotocol-connect-disconnect>>)
...
. Server
** <- <<wireprotocol-op-crypt-key-callback>>
*** Client -> <<wireprotocol-op-crypt-key-callback>> and continue with step 2
** <- <<wireprotocol-op-accept>> -- Record selected protocol and type, continue with step 5 (attach)
** <- <<wireprotocol-op-accept-data>>
+
...
** <- <<wireprotocol-op-trusted-auth,`op_trusted_auth` -- `90`>> (not documented yet)
...
*** Otherwise, this signals pre-attach auth completed
...
_or_ if used as post-attach auth, connection successful (end of flow)
...
+
If the pre-attach auth flow was previously entered, this will essentially be only a `op_response` with either an error or acceptance (connection success).
