...
An `op_allocate_statement` can only be sent together with a <<wireprotocol-statements-prepare>> operation.
...
Request flushing and response processing can be deferred for `ptype_batch_send` or higher.
...
The response processing can then still be deferred until receiving the response of another action.
...
<<wireprotocol-responses-generic>> -- on success, `p_resp_object` holds the remote event id.

// TODO Document receiving events
