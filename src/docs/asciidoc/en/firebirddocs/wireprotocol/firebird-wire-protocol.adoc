...
Be aware that protocol version greater than 10 are OR'ed with `0x8000` (`FB_PROTOCOL_FLAG`) to differentiate them from newer InterBase protocol versions.
...
=== Identification (connect) [[wireprotocol-databases-attach-identification]]

Requests connection to the server and specifies which protocol versions the client is willing to accept.
...
[#wireprotocol-database-cont-auth]
...
`String` -- `p_list`::
On first authentication from client to server: list of (remaining) plugins known by the client, including the current plugin; +
...
Used to exchange information between the client and server parts of a database encryption key plugin.
The server sends this message, and the client responds with the same message.
...
In protocol 13, this message can only occur after authentication and establishing wire protocol encryption.
...
If this message is received _before_ `op_accept`/`op_accept_data`/`op_cond_accept` (so no protocol version has been confirmed yet), you need to assume protocol 15 behaviour for this message.
...
[float]
====== Additions in protocol 14 and higher
...
* Attaching to a database (`op_attach` -- `19`) -- see also <<wireprotocol-databases-attach-attachment>>
...
After detach the connection is still open, to disconnect send <<wireprotocol-databases-disconnect>> (`op_disconnect`).
...
Create a database.
Create is the same request as <<wireprotocol-databases-attach-attachment>> (`op_attach`), but using `op_create` instead of `op_attach`.
...
`Int32` -- `p_operation`::
Operation code (`op_info_database` -- `40`)
...
`Int32` -- `p_operation`::
Operation code (`op_info_transaction` -- `42`)
...
When allocating and preparing together, the value of the statement handle of the _prepare_ must be `0xFFFF` (_invalid object_ handle).
...
* Documented protocol 13 connect/authentication/attach: `op_accept_data`, `op_cond_accept`, `op_cont_auth`
