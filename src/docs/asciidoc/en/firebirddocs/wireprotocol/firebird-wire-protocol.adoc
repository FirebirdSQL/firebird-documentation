...
Some operations have additional mechanisms to handle truncation, like `isc_info_sql_sqlda_start` in statement information requests.
+
The buffer in the response is sized to the actual length of the response (up to the declared available length), so specifying a larger than necessary size does not inflate the response on the wire.
...
|MR
a|* Rendering fixes
* Add missing sentence fragment in <<wireprotocol-requests-info>>
