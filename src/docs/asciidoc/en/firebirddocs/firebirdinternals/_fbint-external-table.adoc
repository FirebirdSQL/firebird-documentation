...
* The <<fbint-exttbl-null-bitmap,null bitmap>> is not written in the file, but the length of the null bitmap needs to be counted in the virtual position in the row
* Columns must be <<fbint-exttbl-align,aligned>>, based on their virtual position, depending on the datatype
* The <<fbint-exttbl-align,alignment>> of the first column must not be written to the file, but does count in the virtual position.

We're using the term _virtual position [in the row]_ to reflect it is not the actual position relative to the start of the row in the file on disk.
In essence, it is the position in the in-memory image of the row, and includes a prefix that is not written to the external table file.

[#fbint-exttbl-null-bitmap]
==== Null Bitmap

In memory (and on <<fbint-page-5, a data page>>) a row has a null bitmap, which is used to determine which rows are `NULL`.
Columns in an external table cannot be `NULL`, and the null bitmap of a row is therefor not written.

Unfortunately, the length of the null bitmap *must* is counted in the virtual position to determine the <<fbint-exttbl-align,alignment>> of columns.
...
The padding to achieve alignment of the first column *must not* be written to disk, but it *must* be counted in the virtual position.
...
.Where
[horizontal.compact]
_n_:: number of bytes (the _n_ in `BINARY(_n_)`)
...
.Example of `ABCD` in `VARCHAR(5) CHARACTER SET ASCII`
----
04 00 41 42 43 44 00
----

.Example of `ABCD` in `VARCHAR(5) CHARACTER SET UTF8`
----
...
.Where
[horizontal.compact]
_n_:: number of bytes (the _n_ in `VARBINARY(_n_)`)
...
[#fbint-exttbl-example]
=== Examples

The following examples are intended to illustrate alignment requirements.

Say we define the following external table:

[listing]
----
create table EXAMPLE_1 external 'example_1.dat' (
  col_1 char(1) character set ASCII,
  col_2 integer
);
----

An `INTEGER` needs to be aligned at multiples of 4, and this results in the following row format:
...
\-> 00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F
  + -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
00| XX XX XX XX 11 00 00 00 22 22 22 22
--/ -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --

/-------------> 00 01 02 03 04 05 06 07
\ actual position on disk
----

Where:

[horizontal.compact]
`XX`:: Null-bitmap (not written)
`11`:: Data for `col_1`
`00`:: Padding bytes for alignment (written)
`22`:: Data for `col_2`

If instead we define `col_1` as `CHAR(4) CHARACTER SET ASCII`, the padding is absent:
...
\-> 00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F
  + -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
00| XX XX XX XX 11 11 11 11 22 22 22 22
--/ -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
...
With a `BIGINT` as the first column, we need to align it to start at virtual position 8:

[listing]
----
create table EXAMPLE_2 external 'example_2.dat' (
  col_1 BIGINT
);
----
...
\-> 00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F
  + -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
00| XX XX XX XX YY YY YY YY 11 11 11 11 11 11 11 11
--/ -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
...
[horizontal.compact]
`XX`:: Null-bitmap (not written)
`YY`:: Padding for `col_1` (not written)
`11`:: Data for `col_1`

In the following two examples, we show the impact of the null-bitmap on alignment.

[listing]
----
create table EXAMPLE_3 external 'example_3.dat' (
  col_1 char(1) character set ASCII,
  col_2 bigint
);
----

This results in the following row format:
...
\-> 00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F
  + -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
00| XX XX XX XX 11 00 00 00 22 22 22 22 22 22 22 22
--/ -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
...
If you define a similar table with 32 more rows, the null bitmap is 8 bytes:

[listing]
----
create table EXAMPLE_4 external 'example_4.dat' (
  col_1 char(1) character set ASCII,
  col_2 bigint,
  col_3 boolean,
  ...
...
\-> 00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F
  + -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
00| XX XX XX XX XX XX XX XX 11 00 00 00 00 00 00 00
10| 22 22 22 22 22 22 22 22 33 ..
--/ -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
...
`22`:: Data for `col_2`
`33`:: Data for `col_3`
`..`:: Remaining columns left out

As you can see, the padding between `col_1` and `col_2` is now 7 bytes, instead of the 3 bytes in the previous example, even though those initial columns have the same definition.
The two columns now take up 16 bytes of space, instead of 12.
This is the impact of the size of the unwritten null-bitmap on the virtual position and the alignment.
