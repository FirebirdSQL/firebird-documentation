...
`Firebird-5.0.0.bbbbb-p-windows-x64.exe`:: Windows executable installer, 64 bits
`Firebird-5.0.0.bbbbb-p-windows-x64.zip`:: Windows zip kit for manual installation
`Firebird-5.0.0.bbbbb-p-windows-x86.exe`:: Windows executable installer, 32 bits
`Firebird-5.0.0.bbbbb-p-linux-x64.tar.gz`:: Linux compressed tarball
`Firebird-5.0.0.bbbbb-p-macos-x64.pkg`:: Mac OS-X 64-bit package

++...++where `bbbbb` is the build number (e.g. 1227 for Firebird 5.0.0 Release Candidate 1) and `p` the packaging number (usually 0 or another low one-digit number).
