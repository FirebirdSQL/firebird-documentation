...
Copyright (C) 2009-2025 Firebird Project and all contributing authors, under the https://firebirdsql.org/docs/html/en/licenses/pdl/public-documentation-license.html[Public Documentation License Version 1.0].
Please refer to the <<gbak-license>> in the Appendix.
...
`Gbak` creates a logical backup of the database structure and data.
In Firebird 1.5 and earlier, it is the only backup utility, while Firebird 2.0 and later also have the `nbackup` utility which is described in
...
`Nbackup` creates a physical backup of the database pages.
...
The `-{wj}CRYPT` option should usually be combined with the <<gbak-cmdline-keyholder>> and <<gbak-cmdline-keyname>> options.
...
This need not be supplied  for embedded mode, if `ISC_PASSWORD` environment variable exists and has the correct value, or if <<gbak-cmdline-fetch-password>> is specified.
...
Perform backup or restore through services manager.
...
This switch causes `gbak` to backup or restore a remote database via the services manager.
...
Allows the username of the SYSDBA or database owner to be specified if the database is to be backed up, or, in the case of a restore (with the `-{wj}c[reate]` switch specified), any valid username can be specified.
...
Write <<gbak-cmdline-verify>> or <<gbak-cmdline-verbint>> output to a log file.
...
Used in conjunction with the <<gbak-cmdline-verify>> or <<gbak-cmdline-verbint>> switch to redirect status messages to a file or device, rather than the screen, or to suppress them altogether.

If `-{wj}y suppress` is used, then no information will be written to screen regardless of whether `-{wj}v[erify]` is specified.

If a filename is given _and_ the <<gbak-cmdline-verify>> or <<gbak-cmdline-verbint>> switch is specified, the file will be written to record progress, errors etc.

[WARNING]
====
All previously known documentation on this switch mentions that the text should be `"suppress_output"` rather than `"suppress"`.
...
This option displays information about the version of `gbak`, and -- optionally -- the server and database.
...
This method does not attempt to back up any databases and does not require a username and password to be defined or supplied.

If you wish to display the version number of the `gbak` utility and also details of the server and database, you must supply a valid database name _and_ backup filename, as follows:
...
[NOTE]
====
When running `gbak`, if the _first_ filename is a database name or database alias, `gbak` will default to taking a backup of the database in transportable format.
...
When this backup file is restored, the tables that were external in the original database will now be normal tables.
...
Prevents the _database triggers_ from firing during a backup.
...
// TODO Would it be beneficial to disable RLE compression (-EXPAND), or does this happen automatically?
...
[NOTE]
====
When running a `gbak` command, if the _first_ filename is a database backup filename then `gbak` will default to running a restore of the database provided that you specify one of `-{wj}c[create database]`, `-{wj}rep[lace_database]` or `-{wj}r[ecreate_database]` in order to make your intentions clear.
...
Either this switch, <<gbak-cmdline-replace-database>>, or <<gbak-cmdline-recreate-database>> must be specified.
...
This switch sets the database cache size (in number of database pages) for the database being restored.
...
To activate indexes later, you need to use
ifdef::backend-pdf[https://firebirdsql.org/file/documentation/pdf/en/refdocs/fblangref50/firebird-50-language-reference.pdf#fblangref50-ddl-idx-altridx[`ALTER INDEX _index-name_ ACTIVE`^].]
ifndef::backend-pdf[https://firebirdsql.org/file/documentation/html/en/refdocs/fblangref50/firebird-50-language-reference.html#fblangref50-ddl-idx-altridx[`ALTER INDEX _index-name_ ACTIVE`].]
...
For example, if you use _Winzip_ or similar tools to create a compressed copy of a database, and you do it when users are accessing the system, the chances of a successful restore of that database are slim.
You must either always use the `gbak` or `nbackup` tools which know how the database works, enable the database backup mode with `ALTER DATABASE BEGIN BACKUP` (and end it with `ALTER DATABASE END BACKUP`), or, use `gfix` to shut the database down completely before you even attempt to back up the database file(s).
...
Those copies can only ever be used to restore a database on the same platform as the one copied.
...
The first is to prevent garbage collection from being carried out while the backup is running.
...
The second option is to back up the database using the `-{wj}se[rvice]` option.
...
Backups taken with the `gbak` application from one version of Firebird can be used by later versions of Firebird to restore the database, however, while this may result in an upgrade to the On-Disk Structure (ODS) for the database in question, the SQL dialect will not be changed.
If you back up a Firebird 1.0 dialect 1 database and then use the backup file to recreate it under Firebird 2.1, for example, the ODS will be updated to 11.1 but the SQL dialect will remain as 1.
...
In practice, simply changing the dialect this way is not sufficient: definitions of tables, stored procedures etc., and your application code will need to be modified to account for difference between dialects.
...
Restoring a database, from a backup, can be made to execute quicker if the `-{wj}se[rvice]` option is used.
...
Such limits could be the size of a CD, the 2 GB limit on individual file sizes on a DVD, where some Unix file systems have a 2 GB limit and so on.
...
The database cache (page buffer) is set when the database is created, or subsequently by using `gfix -{wj}b[uffers] <number-of-pages>`.
`Gbak` can restore a database and set the cache size as well.
...
The default cache size configured in `firebird.conf` is used when the number of buffers is zero, as in the first example above.
...
Similar to the example above to change the database cache size, the database page size can also be changed using `gbak`.
...
However, you can also use the same tools, over an SSH connection to a remote server, and pass the backup of one database directly to the restore of a separate one.

The first example copies a local database to a remote server where Firebird is running and the `firebird` user has its environment set up so that `gbak` is on `$PATH` by default, on login.
...
The final example shows how to back up a remote database on one server, to a remote database on another.
...
* Double quotes around the database filename are not permitted -- it doesn't work for the Firebird utilities either.
...
|17 Apr 2024
|MR
|Moved and reworded `-{wj}NODBTRIGGERS` as it is a backup switch, not a restore switch (https://github.com/FirebirdSQL/firebird-documentation/issues/207[#207])
...
The Original Documentation is titled _Firebird Backup & Restore Utility_.
