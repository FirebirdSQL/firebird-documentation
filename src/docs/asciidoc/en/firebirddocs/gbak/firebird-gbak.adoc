...
When used in a restore, only the database metadata are restored, and any data in the dump file will not be restored.
...
You cannot, for example, take a dump file from Linux over to a Windows server, or from a little-endian system to a big-endian system.
