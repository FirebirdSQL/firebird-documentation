...
For example, a database created with `-REPLICA READ_ONLY` is still writable by the replicator connection, while a database created with `-MODE READ_ONLY` is not writable at all.
====
