...
`-?`::
This switch displays the commandline options and switches.
+
...
In a backup, only the database metadata are backed up.
...
`-SKIP_D[ATA] <regex>`::
Skip backup or restore of data for table(s) matching the SQL regular expression argument.
Opposite of `-INCLUDE[_DATA]`.
// TODO: Add more information
+
NOTE: Introduced in Firebird 3.0.

`-INCLUDE[_DATA] <regex>`::
Backup or restore data for table(s) matching the SQL regular expression argument.
Opposite of `-SKIP_D[ATA]`.
// TODO: Add more information
+
NOTE: Introduced in Firebird 5.0.
...
+
This is a very basic RLE compression, for better compression, use `-ZIP`.
...
It is this format which allows a dump taken on a server of one type to be successfully restored on a server of another type.
...
The option `<access>` is one of:
[horizontal]
`READ_ONLY`::: Read-only database
`READ_WRITE`::: Read/write database
...
Database triggers are a feature of Firebird 2.0 and higher, and are different from _table triggers_ which will continue to fire.
+
...
Backups taken with the `gbak` application from one version of Firebird -- or InterBase -- can be used by later versions of Firebird to restore the database, however, while this may result in an upgrade to the On Disk Structure (ODS) for the database in question, the SQL Dialect will never be changed.
