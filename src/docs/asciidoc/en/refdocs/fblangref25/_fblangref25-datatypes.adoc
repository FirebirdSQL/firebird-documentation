...
|A data type of a dynamically variable size for storing large amounts of data, such as images, text, digital sounds.
...
[[fblangref25-datatypes-inttypes]]
== Integer Data Types
...
The usage and numerical value ranges of hexadecimal notation are described in more detail in the discussion of <<fblangref25-commons-number-constant,number constants>> in the chapter entitled [ref]_Common Language Elements_.
...
It is important to realise that the [replaceable]`s` argument in this template is _scale_, rather than "`a count of digits after the decimal point`".
Understanding the mechanism for storing and retrieving fixed-point data should help to visualise why: for storage, the number is multiplied by 10^s^ (10 to the power of [replaceable]`s`), converting it to an integer;  when read, the integer is converted back.
...
[[fblangref25-datatypes-numerictbl]]
...
[[fblangref25-datatypes-decimaltbl]]
.`NUMERIC` Type Parameters
[cols="<1,<3", options="header",stripes="none"]
|===
^| Parameter
...
[[fblangref25-datatypes-datetime]]
...
See also the <<fblangref25-functions-scalarfuncs-extract,`EXTRACT()` function>> in the chapter entitled [ref]_Built-in Functions and Variables_.

[[fblangref25-datatypes-timestamp]]
=== `TIMESTAMP`
...
[[fblangref25-datatypes-datetimeops]]
...
|`{plus}`
|Numeric value `n`
|`DATE` increased by `n` whole days.
...
|`{plus}`
|Numeric value `n`
|`TIME` increased by `n` seconds.
The fractional part is taken into account

|`TIMESTAMP`
|`{plus}`
|Numeric value `n`
|`TIMESTAMP`, where the date will advance by the number of days and part of a day represented by number [replaceable]`n` -- so "```+ 2.75```" will push the date forward by 2 days and 18 hours
...
|`-`
|Numeric value `n`
|`TIMESTAMP` where the date will decrease by the number of days and part of a day represented by number [replaceable]`n` -- so "```- 2.25```" will decrease the date by 2 days and 6 hours
|===
...
.See also
<<fblangref25-functions-scalarfuncs-dateadd,`DATEADD`>>, <<fblangref25-functions-scalarfuncs-datediff,`DATEADD`>>

[[fblangref25-datatypes-chartypes]]
...
[[fblangref25-datatypes-chartypes-unicode]]
=== Unicode
...
[[fblangref25-datatypes-chartypes-client]]
...
[[fblangref25-datatypes-chartypes-special]]
...
[[fblangref25-datatypes-chartypes-collation]]
=== Collation Sequence
...
A `COLLATE [replaceable]``collation``` clause can be applied in other contexts besides the column definition.
...
[[fblangref25-datatypes-chartypes-caseinsenstv]]
...
.See also
<<fblangref25-commons-predcontaining,`CONTAINING`>>

[[fblangref25-datatypes-chartypes-utf8collations]]
==== `UTF8` Collation Sequences

The following table shows the possible collation sequences for the `UTF8` character set.

[[fblangref25-dtyp-tbl-utfcollats]]
...
[[fblangref25-datatypes-chartypes-charindxs]]
=== Character Indexes
...
[[fblangref25-dtyp-tbl-charindxsz]]
...
.See also
<<fblangref25-ddl-db-create,`CREATE DATABASE`>>, <<fblangref25-datatypes-chartypes-collation,Collation sequence>>, <<fblangref25-dml-select,`SELECT`>>, <<fblangref25-dml-select-where,`WHERE`>>, <<fblangref25-dml-select-groupby,`GROUP BY`>>, <<fblangref25-dml-select-orderby,`ORDER BY`>>

[[fblangref25-datatypes-chartypes-detail]]
=== Character Types in Detail

[[fblangref25-datatypes-chartypes-char]]
==== `CHAR`
...
[[fblangref25-datatypes-chartypes-varchar]]
==== `VARCHAR`
...
[[fblangref25-datatypes-chartypes-nchar]]
==== `NCHAR`
...
[[fblangref25-datatypes-bnrytypes]]
== Binary Data Types
...
.Segment Size
Specifying the BLOB segment is throwback to times past, when applications for working with BLOB data were written in C (Embedded SQL) with the help of the _gpre_ pre-compiler.
...
[[fblangref25-datatypes-bnrytypes-sub]]
=== BLOB Subtypes
...
[[fblangref25-datatypes-bnrytypes-more]]
=== `BLOB` Specifics
...
.See also
<<fblangref25-ddl-filter,`FILTER`>>, <<fblangref25-ddl-filter-declare,`DECLARE FILTER`>>

[[fblangref25-datatypes-array]]
=== `ARRAY` Type
...
Arrays can be one-dimensional and multidimensional and of any data type except `BLOB` and `ARRAY`.
...
[[fblangref25-datatypes-array-bounds]]
...
[[fblangref25-datatypes-array-adddim]]
...
[[fblangref25-datatypes-special]]
== Special Data Types

"`Special`" data types ...

[[fblangref25-datatypes-special-sqlnull]]
=== `SQL_NULL` Data Type
...
[[fblangref25-datatypes-conversion]]
...
[[fblangref25-datatypes-convert-explicit]]
...
[[fblangref25-datatypes-convert-typeof]]
...
[[fblangref25-datatypes-convert-conversions]]
==== Conversions Possible for the `CAST` Function

[[fblangref25-dtyp-tbl-conversions]]
...
[[fblangref25-datatypes-convert-literalformats]]
==== Literal Formats

To cast string data types to the `DATE`, `TIME` or `TIMESTAMP` data types, you need the string argument to be one of the predefined date and time literals (see xref:fblangref25-dtyp-tbl-keydesc[xrefstyle="short"]) or a representation of the date in one of the allowed _date-time literal_ formats:
...
[[fblangref25-dtyp-tbl-predliterals]]
...
[[fblangref25-datatypes-convert-shortcasts]]
...
[[fblangref25-datatypes-convert-implicit]]
...
[[fblangref25-datatypes-convert-implicit-concat]]
...
[[fblangref25-datatypes-domainattribs]]
=== Domain Attributes
...
.See also
<<fblangref25-datatypes-convert-typeof,Explicit Data Type Conversion>> for the description of differences in the data conversion mechanism when domains are specified for the `TYPE OF` and `TYPE OF COLUMN` modifiers.

[[fblangref25-datatypes-domainoverride]]
=== Domain Override
...
[[fblangref25-dtyp-tbl-domoverride]]
...
[[fblangref25-datatypes-domaincreate]]
...
.See also
<<fblangref25-ddl-domn-create,`CREATE DOMAIN`>> in the Data Definition Language (DDL) section.

[[fblangref25-datatypes-domainalter]]
==== Altering a Domain
...
When planning to alter a domain, its dependencies must be taken into account: whether there are table columns, any variables, input and/or output parameters with the type of this domain declared in the PSQL code.
...
.See also
<<fblangref25-ddl-domn-alter,`ALTER DOMAIN`>> in the Data Definition Language (DDL) section.

[[fblangref25-datatypes-domaindrop]]
...
.See also
<<fblangref25-ddl-domn-drop,`DROP DOMAIN`>> in the Data Definition Language (DDL) section.
