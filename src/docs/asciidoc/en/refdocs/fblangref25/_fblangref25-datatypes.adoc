...
The `FLOAT` data type has an approximate precision of 7 decimal digits.
To ensure the safety of storage, rely on 6 digits.

[[fblangref25-datatypes-double]]
=== `DOUBLE PRECISION`

The `DOUBLE PRECISION` data type is stored with an approximate precision of 15 decimal digits.
...
.See also
<<fblangref25-functions-scalarfuncs-dateadd,`DATEADD`>>, <<fblangref25-functions-scalarfuncs-datediff,`DATEDIFF`>>
