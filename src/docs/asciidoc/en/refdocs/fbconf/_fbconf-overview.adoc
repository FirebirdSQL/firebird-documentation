...
<<fbconf-trace>>::
Global and per-database trace configuration (system configuration example in `fbtrace.conf`, users may start traces using the services API using the same syntax)
