...
<<fbconf-firebird>>::
Global configuration of Firebird server and/or client library

<<fbconf-databases>>::
Database aliases, and per-database configuration of Firebird server and/or client library

<<fbconf-plugins>>::
Plugin configuration of Firebird server and/or client library

<<fbconf-replication>>::
Global and per-database replication configuration of Firebird server

<<fbconf-trace>>::
Global and per-database trace configuration of Firebird server

<<fbconf-environment-variables>>::
Environment variables to override parts of Firebird server and/or client configuration
...
scoped values::
Some configuration files (<<fbconf-databases>>, <<fbconf-plugins>>, <<fbconf-replication>>, <<fbconf-trace,trace configuration>>) allow or require keys with a string value followed by additional key-value pairs enclosed in braces.
+
...
Unquoted, the '```++#++```' is interpreted as the start of a <<fbconf-syntax-comment,comment>>, and the braces as the start or end of a <<fbconf-syntax-scope,scoped value>>.
...
[listing]
----
<key-or-keyword> [ = _simple-value_ ]
'{'
  { _key-name_ = <value> <new-line> }...
'}'
----

[NOTE]
====
In syntax descriptions, the unquoted braces are used to define a syntax group (a group of tokens).
Above they're used with `++...++` to signify that the syntax group is repeatable.
In syntax descriptions, we're using `++'{'++` and `++'}'++` to mean literal braces, to distinguish them from syntax groups.
====

In most cases, _value_ is a simple value, but for example in <<fbconf-replication>> it can also be a scoped value.

We call the part after the `=` (equals) a __scoped value__footnote:scoped[The term _scoped value_ is specific to this documentation, other Firebird documentation do not use this term].
...
these are covered in <<fbconf-replication>>.
