...
If a client is idle for more than the configured interval, the server will send a dummy packet to the client.
This packet will result in a TCP/IP-level acknowledgement if the connection is still alive.
Otherwise, it will result in detection of the broken connection.
...
// TODO Try and find a more recent version of this documentation
