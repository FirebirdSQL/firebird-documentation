...
By default `IPv6V6Only` is set to false (`0`), and the Firebird server listens on the zero IPv6 address (`::`) and accepts all incoming connections, whether IPv4 or IPv6.
If `IPv6V6Only` is set to true (`1`), the server, still listening implicitly or explicitly on the zero IPv6 address, will accept only IPv6 connections.
