...
. Connection level (through https://firebirdsql.org/file/documentation/chunk/en/refdocs/fblangref50/fblangref50-management-timeouts.html#fblangref50-management-setstatementtimeout[`SET STATEMENT TIMEOUT`] or the API)
+
...
A non-zero statement-level timeout takes precedence over the connection-level timeout (i.e. the connection-level timeout is not an upper bound).
