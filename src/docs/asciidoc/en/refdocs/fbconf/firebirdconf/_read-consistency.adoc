...
Since Firebird 4.0, `READ COMMITTED` by default always uses the https://firebirdsql.org/file/documentation/release_notes/html/en/4_0/rlsnotes40.html#rnfb40-engine-stmt-level-consistency[`READ CONSISTENCY` mode], even if `[NO] RECORD VERSION` (`isc_tpb_rec_version`/`isc_tpb_no_rec_version`) is requested.
