...
+
If environment variable <<fbconf-env-isc-path>> is not set, the paths listed are used to resolve filenames *without* a path component, including a check for existence.
The first entry in the list is used as the fallback if no such file exists.
...
.See also
<<fbconf-external-file-access>>, <<fbconf-env-isc-path>>
