...
The connection idle timeout -- or session idle timeout -- is the maximum time a connection may be idle (i.e. the database engine receives no requests) before it shuts down the connection.
...
The connection idle timeout can be configured on two levels:

. Database level (through `firebird.conf` or `databases.conf`)
. Connection level (through https://firebirdsql.org/file/documentation/chunk/en/refdocs/fblangref50/fblangref50-management-timeouts.html#fblangref50-management-setsessionidle[`SET SESSION IDLE TIMEOUT`] or the API)
+
