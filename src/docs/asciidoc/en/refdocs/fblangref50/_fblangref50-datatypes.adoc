[[fblangref50-datatypes]]
= Data Types and Subtypes

Data of various types are used to: 

* Define columns in a database table in the <<fblangref50-ddl-tbl-create,`CREATE TABLE`>> statement or change columns using <<fblangref50-ddl-tbl-alter,`ALTER TABLE`>>
* Declare or change a _domain_ using the <<fblangref50-ddl-domn-create,`CREATE DOMAIN`>> or <<fblangref50-ddl-domn-alter,`ALTER DOMAIN`>> statements
* Declare local variables, return values and parameters in <<fblangref50-psql,PSQL modules>> and UDFs -- user-defined functions
* Provide arguments for the <<fblangref50-scalarfuncs-cast,`CAST()`>> function when explicitly converting data from one type to another
...
|===
|<<fblangref50-scalarfuncs-abs,ABS>>
|<<fblangref50-scalarfuncs-ceil,CEILING>>
|<<fblangref50-scalarfuncs-exp,EXP>>
|<<fblangref50-scalarfuncs-floor,FLOOR>>
|<<fblangref50-scalarfuncs-ln,LN>>
|<<fblangref50-scalarfuncs-log,LOG>>
|<<fblangref50-scalarfuncs-log10,LOG10>>
|<<fblangref50-scalarfuncs-power,POWER>>
|<<fblangref50-scalarfuncs-sign,SIGN>>
|<<fblangref50-scalarfuncs-sqrt,SQRT>>
|===

The aggregate functions <<fblangref50-aggfuncs-sum,`SUM`>>, <<fblangref50-aggfuncs-avg,`AVG`>>, <<fblangref50-aggfuncs-max,`MAX`>> and <<fblangref50-aggfuncs-min,`MIN`>> work with `DECFLOAT` data, as do all the <<fblangref50-aggfuncs-stats,statistical aggregates>> (including but not limited to `STDDEV` or `CORR`).
...
* <<fblangref50-contextvars-current-time,`CURRENT_TIME`>> and <<fblangref50-contextvars-localtime,`LOCALTIME`>> default to seconds precision and can be specified up to milliseconds precision with `++CURRENT_TIME({0|1|2|3})++` or `++LOCALTIME({0|1|2|3})++`
* <<fblangref50-contextvars-current-timestamp,`CURRENT_TIMESTAMP`>> and <<fblangref50-contextvars-localtimestamp,`LOCALTIMESTAMP`>> default to milliseconds precision.
...
* Functions <<fblangref50-scalarfuncs-dateadd,`DATEADD()`>> and <<fblangref50-scalarfuncs-datediff,`DATEDIFF()`>> support up to deci-milliseconds precision with `MILLISECOND`
* The <<fblangref50-scalarfuncs-extract,`EXTRACT()` function>> returns up to deci-milliseconds precision with the `SECOND` and `MILLISECOND` arguments
...
In Dialect 1, `DATE` is an alias for <<fblangref50-datatypes-timestamp>>.
Dialect 1 has no "`date-only`" type. 
...
If you need to get the time-part from `DATE` in Dialect 1, you can use the <<fblangref50-scalarfuncs-extract,`EXTRACT` function>>.
...
See also the <<fblangref50-scalarfuncs-extract,`EXTRACT()` function>> in the chapter entitled _Built-in Scalar Functions_.
...
In Dialect 1, <<fblangref50-datatypes-date>> is an alias for `TIMESTAMP`.

The <<fblangref50-scalarfuncs-extract,`EXTRACT` function>> works equally well with `TIMESTAMP` as with the Dialect 1 `DATE` type.
...
Subsequently, the time zone can be changed to a given time zone using a <<fblangref50-management-settimezone,`SET TIME ZONE`>> statement or reset to its original value with `SET TIME ZONE LOCAL` or <<fblangref50-management-session-reset-alter,`ALTER SESSION RESET`>>.
...
`CHAR CHARACTER SET OCTETS` has the alias <<fblangref50-datatypes-chartypes-binary>>.
...
`VARCHAR CHARACTER SET OCTETS` has the alias <<fblangref50-datatypes-chartypes-varbinary>>.
...
|<<fblangref50-commons-predbetween,`BETWEEN`>>,
|<<fblangref50-commons-isnotdistinct,`IS [NOT] DISTINCT FROM`>>,
|<<fblangref50-commons-in,`IN`>>,
|<<fblangref50-commons-quant-anysome,`ANY` {vbar} `SOME`>>,
|<<fblangref50-commons-quant-all,`ALL`>>
|{nbsp}
|===

As an efficient alternative to concatenation, you can also use <<fblangref50-scalarfuncs-blob-append,`BLOB_APPEND()`>> or the functions and procedures of system package <<fblangref50-sys-pckg-blobutil,`RDB$BLOB_UTIL`>>.
...
|===
|<<fblangref50-commons-predstartwith,`STARTING [WITH]`>>,
...
.See also
<<fblangref50-ddl-filter,`FILTER`>>, <<fblangref50-ddl-filter-declare,`DECLARE FILTER`>>, <<fblangref50-scalarfuncs-blob-append,`BLOB_APPEND()`>>, <<fblangref50-sys-pckg-blobutil,`RDB$BLOB_UTIL`>>
...
This data type exists to support the use of untyped parameters in expressions involving the <<fblangref50-commons-isnotnull,`IS NULL`>> predicate.
...
The <<fblangref50-scalarfuncs-cast,`CAST` function>> enables explicit conversion between many pairs of data types.
...
[#fblangref50-datatypes-convert-implicit-compare]
...
xref:fblangref50-dtyp-tbl-domoverride[xrefstyle="short"] summarises the rules for domain override.
...
A domain is created with the DDL statement <<fblangref50-ddl-domn-create,`CREATE DOMAIN`>>.
...
.See also
<<fblangref50-ddl-domn-create,`CREATE DOMAIN`>> in the _Data Definition (DDL) Statements_ chapter.

[[fblangref50-datatypes-domainalter]]
==== Altering a Domain

To change the attributes of a domain, use the DDL statement <<fblangref50-ddl-domn-alter,`ALTER DOMAIN`>>.
...
.See also
<<fblangref50-ddl-domn-alter,`ALTER DOMAIN`>> in the _Data Definition (DDL) Statements_ chapter.

[[fblangref50-datatypes-domaindrop]]
==== Deleting (Dropping) a Domain

The DDL statement <<fblangref50-ddl-domn-drop,`DROP DOMAIN`>> deletes a domain from the database, provided it is not in use by any other database objects.
...
.See also
<<fblangref50-ddl-domn-drop,`DROP DOMAIN`>> in the _Data Definition (DDL) Statements_ chapter.
