...
The behaviour of `DECFLOAT` operations in a session, specifically rounding and error behaviour, can be configured using the <<fblangref50-management-setdecfloat,`SET DECFLOAT`>> management statement.
...
<<fblangref50-scalarfuncs-comparedecfloat,`COMPARE_DECFLOAT`>>:: compares two `DECFLOAT` values to be equal, different or unordered
<<fblangref50-scalarfuncs-normalizedecfloat,`NORMALIZE_DECFLOAT`>>:: takes a single `DECFLOAT` argument and returns it in its simplest form
<<fblangref50-scalarfuncs-quantize,`QUANTIZE`>>:: takes two `DECFLOAT` arguments and returns the first argument scaled using the second value as a pattern
<<fblangref50-scalarfuncs-totalorder,`TOTALORDER`>>:: performs an exact comparison on two `DECFLOAT` values
...
Firebird 4.0 introduced time zone support, using the types `TIME WITH TIME ZONE` and `TIMESTAMP WITH TIME ZONE`.
...
[[fblangref50-datatypes-time-zone-util-version]]
...
[[fblangref50-datatypes-timezone-zone-util-transitions]]
...
.See also
<<fblangref50-ddl-db-create,`CREATE DATABASE`>>, <<fblangref50-datatypes-chartypes-collation,Collation sequence>>, <<fblangref50-dml-select,`SELECT`>>, <<fblangref50-dml-select-where,`WHERE`>>, <<fblangref50-dml-select-groupby,`GROUP BY`>>, <<fblangref50-dml-select-orderby,`ORDER BY`>>
