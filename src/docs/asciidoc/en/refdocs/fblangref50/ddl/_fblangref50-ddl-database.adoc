...
`USER` and `PASSWORD`::
Clauses for specifying the username and the password, respectively, of an existing user in the security database (`security4.fdb` or whatever is configured in the _SecurityDatabase_ configuration).
