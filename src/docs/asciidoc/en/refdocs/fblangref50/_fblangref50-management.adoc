...
Examples are the `++ALTER DATABASE {BEGIN | END} BACKUP++` statements to control the "`copy-safe`" mode, or the `ALTER EXTERNAL CONNECTIONS POOL` statements introduced in Firebird 4.0 to manage connection pooling.
...
Although some <<fblangref50-ddl-db-alter,`ALTER DATABASE`>> clauses (`BEGIN BACKUP`) can be considered as management statements, they are documented in the _DDL_ chapter.
====
...
The `3.0` option maps data types introduced after Firebird 3.0 -- in particular `DECIMAL`/`NUMERIC` with precision 19 or higher, `INT128`, `DECFLOAT`, `TIME/TIMESTAMP WITH TIME ZONE` -- to data types supported in Firebird 3.0.
...
[NOTE]
====
For more information about the API calls, consult the https://www.firebirdsql.org/file/documentation/release_notes/html/en/4_0/rlsnotes40.html[_Firebird 4.0 Release Notes_].
====
