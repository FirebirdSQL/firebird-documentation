...
[[fblangref50-functions-nameclashes]]
...
To make the internal function available, you need either to <<fblangref50-ddl-extfunc-drop,`DROP`>> the UDF, or to use <<fblangref50-ddl-extfunc-alter,`ALTER EXTERNAL FUNCTION`>> to change the declared name of the UDF.
****
...
YES -> <<fblangref50-functions-nameclashes,Read details>>
...
.Possible name conflict
YES -> <<fblangref50-functions-nameclashes,Read details>> (Affects `CEILING` only)
