...
However, this does not apply to global temporary tables (GTT), which are allowed to be changed in `READ ONLY` transactions, see <<fblangref50-ddl-tbl-gtt,_Global Temporary Tables (GTT)_>> in Chapter _Data Definition (DDL) Statements_ for details.
...
Or a web service may dispatch distributed sub-services doing processing in parallel.
...
To retrieve the updated list of rows in the table you are interested in -- "`refresh`" -- the `SELECT` statement needs to be requested again, whilst still in the uncommitted `READ COMMITTED` transaction.
...
This is a limitation that prevents dynamic specification of transaction names and thus, rules out transaction naming in DSQL.
====
* The optional keyword `WORK` is supported for compatibility with other relational database management systems that require it.
...
+ 
For soft-committed transactions whose isolation level is `SNAPSHOT` or `SNAPSHOT TABLE STABILITY`, the view of database state does not update to reflect changes by other transactions, and the user of the application instance continues to have the same view as when the transaction started originally.
