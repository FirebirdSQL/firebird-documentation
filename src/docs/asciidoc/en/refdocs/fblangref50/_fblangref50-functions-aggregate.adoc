...
[NOTE]
====
The `DISTINCT` parameter makes no sense if used with `MAX()` as it doesn't change the result;
it is implemented only for compliance with the standard.
====
...
[NOTE]
====
The `DISTINCT` parameter makes no sense if used with `MIN()` as it doesn't change the result;
it is implemented only for compliance with the standard.
====
