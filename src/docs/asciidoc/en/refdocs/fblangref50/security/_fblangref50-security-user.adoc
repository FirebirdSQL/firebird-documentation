...
* When the `AUTO ADMIN MAPPING` flag is enabled in the security database (`{secdb}` or whatever is the security database configured for the current database in the `databases.conf`), any Windows Administrator - assuming `Win_Sspi` was used to connect without specifying roles.
