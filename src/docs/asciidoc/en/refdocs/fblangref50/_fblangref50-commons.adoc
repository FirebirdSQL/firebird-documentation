...
Strings entered this way will be type `BINARY` (or character set `OCTETS`) by default, unless the <<fblangref50-commons-introducer-syntax,[term]_introducer syntax_>> is used to force a string to be interpreted as another character set.
...
Inside the string, i.e. `<char>` items, single quotes (not escaped) can be used.
