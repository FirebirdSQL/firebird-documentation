...
`DOUBLE PRECISION`, `BIGINT`, `TIMESTAMP`, and `BLOB` occupy 8 bytes.
...
For integer data types (`SMALLINT`, `INTEGER`, `BIGINT`), and -- dialect 1 -- `DOUBLE PRECISION`:
...
|SMALLINT
|Length of `CHAR` or `VARCHAR` in characters (not in bytes)
...
|RDB$FIELD_PRECISION
|SMALLINT
|Precision of an exact numeric data type (_p_ in `DECIMAL(p[,s])` and `NUMERIC(p[,s])`).
