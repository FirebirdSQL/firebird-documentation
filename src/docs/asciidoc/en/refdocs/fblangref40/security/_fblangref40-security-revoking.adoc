...
<privileges> ::=
  !! See <<fblangref40-security-grant-privsyntax,`GRANT` syntax>> !!
----
...
<role_granted_list> ::=
  !! See <<fblangref40-security-grant-rolesyntax,`GRANT` syntax>> !!
...
[[fblangref40-security-tbl-revoke]]
...
See <<fblangref40-security-grant,`GRANT`>> for detailed descriptions of the various types of privileges.

Only the user who granted the privilege can revoke it.

[[fblangref40-security-revoke-default]]
=== The `DEFAULT` Clause
...
[[fblangref40-security-revoke-from]]
=== The `FROM` Clause
...
[[fblangref40-security-revoke-grantoption]]
...
[[fblangref40-security-revoke-role]]
...
[[fblangref40-security-revoke-exgrantby]]
...
[[fblangref40-security-revoke-allonall]]
=== Revoking `ALL ON ALL`
...
When the current user is logged in with full <<fblangref40-security-administrators,administrator>> privileges in the database, the `REVOKE ALL ON ALL` will remove all privileges, no matter who granted them.
...
[[fblangref40-security-revoke-exmpl]]
...
.See also
<<fblangref40-security-grant,`GRANT`>>
