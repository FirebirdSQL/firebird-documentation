...
For the `Srp` plugin, the maximum password length is 255 characters, for an effective length of 20 bytes (see also <<fblangref30-security-auth-effective-20-bytes>>).
...
[[fblangref30-security-auth-effective-20-bytes]]
...
[[fblangref30-security-auth-special]]
...
These are discussed in the sections that follow and are summarised in <<fblangref30-security-administrators>>.

[[fblangref30-security-auth-special-posix]]
=== POSIX Hosts
...
[[fblangref30-security-auth-sysdba-posix]]
...
[[fblangref30-security-auth-root-posix]]
==== The _root_ User
...
[[fblangref30-security-auth-special-windows]]
=== Windows Hosts
...
For details, refer to the later section entitled <<fblangref30-security-autoadminmapping>>.
...
In Firebird 3.0 and later, the mapping must be done explicitly using <<fblangref30-security-mapping-create,`CREATE MAPPING`>>.
====

[[fblangref30-security-auth-special-dbowner]]
=== The Database Owner
...
The user who is the owner of a database has full <<fblangref30-security-administrators,administrator privileges>> with respect to that database, including the right to drop it, to restore it from a backup and to enable or disable the <<fblangref30-security-autoadminmapping>> capability.
...
[[fblangref30-security-rdbadmin03]]
...
[[fblangref30-security-tbl-rdbadmin]]
...
The grantor must be logged in as an <<fblangref30-security-administrators,administrator>>.

.See also
<<fblangref30-security-user-create,`CREATE USER`>>, <<fblangref30-security-user-alter,`ALTER USER`>>, <<fblangref30-security-grant,`GRANT`>>, <<fblangref30-security-revoke,`REVOKE`>>

[[fblangref30-security-rdbadmin04]]
...
[[fblangref30-security-rdbadmin05]]
...
[[fblangref30-security-rdbadmin0]]
...
[[fblangref30-security-rdbadmin01]]
...
In order to grant and revoke the `RDB$ADMIN` role, the grantor must be logged in as an <<fblangref30-security-administrators,administrator>>.

.See also
<<fblangref30-security-grant,`GRANT`>>, <<fblangref30-security-revoke,`REVOKE`>>

[[fblangref30-security-rdbadmin02]]
...
[[fblangref30-security-autoadminmapping]]
=== `AUTO ADMIN MAPPING`
...
If an explicit role was specified on connect, the `RDB$ADMIN` role can be assumed later in the session using <<fblangref30-management-role-set-trusted,`SET TRUSTED ROLE`>>.
...
* The database owner
* An <<fblangref30-security-administrators,administrator>>
...
For details, see <<fblangref30-security-mapping>>
====
...
[[fblangref30-security-autoadminmapping02]]
...
[[fblangref30-security-administrators]]
== Administrators
...
[[fblangref30-security-tbl-admins]]
