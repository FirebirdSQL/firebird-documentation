...
[NOTE]
====
In Firebird 3.0 and Firebird 4.0, the `USAGE` privilege is only available for exceptions and sequences (in `gen_id(__gen_name__, __n__) or `next value for __gen_name__`).
