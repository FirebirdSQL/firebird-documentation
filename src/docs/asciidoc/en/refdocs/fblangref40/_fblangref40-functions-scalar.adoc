...
|expr__N__
|An expression of a type convertible to `BLOB SUB_TYPE TEXT`
|===
...
The `BLOB_APPEND` function takes two or more arguments and adds them to a `BLOB SUB_TYPE TEXT` which remains open for further modification by a subsequent `BLOB_APPEND` call.
...
If the original value is a character type, its character set is used (for string literals, the connection character set), otherwise character set `NONE`.
...
The `BLOB_APPEND` function returns a temporary unclosed `BLOB SUB_TYPE TEXT` with the `BLB_close_on_read` flag.
If the first argument is such a temporary unclosed `BLOB` (e.g. created by a previous call to `BLOB_APPEND`), it will be used as-is, otherwise a new `BLOB SUB_TYPE TEXT` is created.
