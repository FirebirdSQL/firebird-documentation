...
[[fblangref40-scalarfuncs-rsasign]]
=== `RSA_SIGN`
...
[listing,subs=+quotes]
----
RSA_SIGN (_message_ KEY _private_key_
...
`RSA_SIGN` performs PSS encoding of the _input_ to be signed, and signs using the RSA private key.
...
[[fblangref40-scalarfuncs-rsasign-exmpl]]
==== `RSA_SIGN` Examples
...
.See also
<<fblangref40-scalarfuncs-rsaverify>>, <<fblangref40-scalarfuncs-rsaprivate>>

[[fblangref40-scalarfuncs-rsaverify]]
=== `RSA_VERIFY`
...
[listing,subs=+quotes]
----
RSA_VERIFY (_input_ SIGNATURE _signature_
  KEY _public_key_
  [HASH <hash>] [SALT_LENGTH _salt_length_])
...
|signature
|Expected signature of _input_ generated by `RSA_SIGN`
...
|Hash to use for the message digest;
default is `SHA256`.
Value must match the hash used in `RSA_SIGN`

|salt_length
|Length of the salt in bytes;
default is 8;
minimum 1, maximum 32.
Value must match the length used in `RSA_SIGN`.
|===

`RSA_SIGN` performs PSS encoding of the _input_ to be verified, and verifies the digital signature using the provided RSA public key.

[[fblangref40-scalarfuncs-rsaverify-exmpl]]
==== `RSA_VERIFY` Examples

[TIP]
====
Run the examples of the <<fblangref40-scalarfuncs-rsaprivate-exmpl,`RSA_PRIVATE`>>, <<fblangref40-scalarfuncs-rsapublic-exmpl,`RSA_PUBLIC`>> and <<fblangref40-scalarfuncs-rsasign-exmpl,`RSA_SIGN`>> functions first.
====
...
.See also
<<fblangref40-scalarfuncs-rsasign>>, <<fblangref40-scalarfuncs-rsapublic>>
