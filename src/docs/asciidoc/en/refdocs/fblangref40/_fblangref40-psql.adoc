...
Statements that start and end transactions are not available in PSQL, but it is possible to run a statement or a block of statements in an <<fblangref40-psql-inauton,autonomous transaction>>.
...
<declare-var> ::=
  !! See <<fblangref40-psql-declare-variable,DECLARE VARIABLE>> !!

<declare-cursor> ::=
  !! See <<fblangref40-psql-declare-cursor, DECLARE .. CURSOR>> !!

<subfunc-forward>, <subfunc-def> ::=
  !! See <<fblangref40-psql-declfunc,DECLARE FUNCTION>> !!

<subproc-forward>, <subproc-def> ::=
  !! See <<fblangref40-psql-declproc,DECLARE PROCEDURE>> !!
----
...
For information on modifying existing package header or bodies, see <<fblangref40-ddl-pkg-alter,`ALTER PACKAGE`>>, <<fblangref40-ddl-pkg-createalter,`CREATE OR ALTER PACKAGE`>>, <<fblangref40-ddl-pkg-recreate,`RECREATE PACKAGE`>>, <<fblangref40-ddl-pkg-body-recreate,`RECREATE PACKAGE BODY`>>
...
[none]
* <<fblangref40-management-session-reset-alter,`ALTER SESSION RESET`>>
* <<fblangref40-management-setbind,`SET BIND`>>
* <<fblangref40-management-setdecfloat,`SET DECFLOAT`>>
* <<fblangref40-management-role-set,`SET ROLE`>>
* <<fblangref40-management-setsessionidle,`SET SESSION IDLE TIMEOUT`>>
* <<fblangref40-management-setstatementtimeout,`SET STATEMENT TIMEOUT`>>
* <<fblangref40-management-settimezone,`SET TIME ZONE`>>
* <<fblangref40-management-role-set-trusted,`SET TRUSTED ROLE`>>
...
.See also
<<fblangref40-management,Management Statements>>
...
<when_do> ::=
  !! See <<fblangref40-psql-when,WHEN ... DO>> !!
----
...
For <<fblangref40-psql-handleexceptions,error handling>>, you can add one or more <<fblangref40-psql-when>> statements immediately before `END`.
...
.See also
<<fblangref40-psql-exit>>, <<fblangref40-sidebar01,`SET TERM`>>, <<fblangref40-psql-when>>
...
See <<fblangref40-psql-inauton>> for more details on autonomous transactions.
...
In PSQL code, exceptions are handled by means of the <<fblangref40-psql-when>> statement.
...
The <<fblangref40-scalarfuncs-rdberror,`RDB$ERROR`>> function can be used the obtain the SQLCODE, GDSCODE, SQLSTATE, custom exception name and exception message.
...
.See also
<<fblangref40-psql-exception>>, <<fblangref40-ddl-exception-create,`CREATE EXCEPTION`>>, <<fblangref40-appx02-tbl-errcodes01,SQLCODE and GDSCODE Error Codes and Message Texts>> and <<fblangref40-appx02-tbl-sqlstates,SQLSTATE Codes and Message Texts>>, <<fblangref40-contextvars-gdscode,`GDSCODE`>>, <<fblangref40-contextvars-sqlcode,`SQLCODE`>>, <<fblangref40-contextvars-sqlstate,`SQLSTATE`>>, <<fblangref40-scalarfuncs-rdberror,`RDB$ERROR()`>>
