...
Column reference::
Identifier or identifier chain of a column from a specified table used in evaluations or as a search condition.
...
Constant or Literal::
Numbers, or string literals enclosed in apostrophes or Q-strings, Boolean values (`TRUE`, `FALSE`, and `UNKOWN`), and `NULL`
...
Function reference::
The identifier or identifier chain of an internal, packaged, stored or external function in a function expression
...
`COLLATE` clause::
Clause applied to character types to specify the character-set-specific collation to use in string comparisons
...
It can be a <<langref-commons-string-constant,string>>, a <<langref-commons-number-constant,number>>, a <<langref-commons-boolean-literal,Boolean>>, a <<langref-commons-datetime-literal,datetime>>, or `NULL`.
...
If you need to convert from a different format than listed below, use the `FORMAT` clause of <<langref-scalarfuncs-cast>>.
...
The concatenation operator -- two pipe characters known as "`double pipe`" or '```||```' -- concatenates two operands of any type convertible to string to form a single string.
Operands can be literals or other expressions.
...
The type of the resulting expression is `BLOB` if one of the operands is `BLOB`, otherwise it's `VARCHAR(_n_)` with _n_ equal to the sum of the declared lengths of both operands.

If an operand is not a string type, its length is an implementation-defined length sufficient to store any of its values.
For example, an `INTEGER` is considered to have a length of 11, for minus-sign and 10 digits, so `1 || 2` is a `VARCHAR(22)`.

The padding of a value of type `CHAR` is preserved during concatenation, so ``'A ' || 'B '`` is a `VARCHAR(4)` with value ``'A B '``.

The character set is:

. `OCTETS` if one of the operands is `BINARY` or `VARBINARY`, or has character set `OCTETS`
. The character set of the first operand, if it is not `NONE`
. Otherwise, the character set of the second operand

A string literal -- without introducer -- has the connection character set as its character set, or `NONE` if no connection character set was specified.
A binary string literal -- without introducer -- has `OCTETS` as its character set.
A non-string value implicitly converted to string has `NONE` as its character set.
...
.Compatibility Note
In the SQL standard, string concatenation is only defined between character and binary string types.
Firebird is more lenient, and will automatically convert any operand of `||` to string.
...
`NEXT VALUE FOR` increments a sequences, and returns the resulting value.
...
Unlike the `GEN_ID` function, the `NEXT VALUE FOR` expression does not accept an increment, so there is no way to retrieve the _current value_ of a sequence, nor to step the next value by a different value than the _increment_ configured for the sequence.
...
A conditional expression returns different values based on its defined conditions.
...
The returned result does not have to be a literal value: it can be any form of expression.
...
As with the simple `CASE` construct, the returned result does not have to be a literal value: it can be any form of expression.
...
.Syntax
[listing#langref-commons-in-syntax,subs="+quotes,macros"]
----
<value> [NOT] IN <in-source>
...
<unlist-table-function> ::=
  !! See <<langref-tblvalfunc-unlist,UNLIST()>> !!
----
...
The `IN UNLIST(...)` is a shorter way of using the table-valued function <<langref-tblvalfunc-unlist>>.
...
-- Equivalent long form UNLIST
select EMP_NO from EMPLOYEE
where DEPT_NO in (select * from unlist('120,900') as depts);
----

Using `UNLIST` with a string literal like this will rarely make sense: it will usually be better (e.g. for performance) to use `IN (<value-list>)`.
It's especially useful with strings obtained elsewhere, like a column or procedure parameter.
...
----
. The same example, but using <<langref-tblvalfunc-unlist,`UNLIST`>> to split a string with #-separated values into separate values:
+
[source]
----
select *
from EMPLOYEE
where FIRST_NAME in unlist('Pete#Ann#Roger', '#');
----
