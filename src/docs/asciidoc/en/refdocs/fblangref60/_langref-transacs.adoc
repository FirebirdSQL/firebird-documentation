...
[#langref-transacs-settransac-releaseblob]
===== `AUTO RELEASE TEMP BLOBID`

This option releases the temporary ID of a user `BLOB` just after its materialisation.
It's useful for mass-insertion of records with user-defined ``BLOB``s because it eliminates the memory overhead caused by retaining the temporary IDs until transaction end.

This option should be used with care.
Only use it if there is no need to access a materialised `BLOB` via a temporary ID obtained after its creation.
For example, do not use this when using `RETURNING *` or `RETURNING somecolumn`, where `somecolumn` is a `BLOB`.

This option is used by _gbak_ during a database restore.
