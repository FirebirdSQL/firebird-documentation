...
However, this does not apply to global temporary tables (GTT), which are allowed to be changed in `READ ONLY` transactions, see <<langref-ddl-tbl-gtt,_Global Temporary Tables (GTT)_>> in xref:fblangref50-ddl[xrefstyle=full] for details.
