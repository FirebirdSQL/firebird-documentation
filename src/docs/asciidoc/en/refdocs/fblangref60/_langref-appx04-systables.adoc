...
Information about shadow files
...
// Keep column description in-sync with langref-appx06-tbl-authmapping

[#langref-appx04-tbl-authmapping]
...
[#langref-appx04-tbl-backuphistory]
[cols="<4m,<3m,<5", frame="all", options="header",stripes="none"]
|===
^| Column Name
^| Data Type
...
[#langref-appx04-tbl-characterset]
[cols="<4m,<3m,<5", frame="all", options="header",stripes="none"]
|===
^| Column Name
^| Data Type
...
[#langref-appx04-tbl-chkconstraints]
[cols="<4m,<3m,<5", frame="all", options="header",stripes="none"]
|===
^| Column Name
^| Data Type
...
|RDB$TRIGGER_NAME
|CHAR(63)
|For `CHECK` constraints, the name of the trigger that enforces the constraint.
For `NOT NULL` constraints, the name of the affected column.
For foreign key constraints, the name of the trigger that enforces the `ON UPDATE` and `ON DELETE` clauses.
|===
...
Together with the character set identifier, it is a unique collation identifier
...
[#langref-appx04-tbl-dependencies]
[cols="<4m,<3m,<5", frame="all", options="header",stripes="none"]
|===
^| Column Name
^| Data Type
...
`RDB$FIELDS` stores definitions of named and system-generated domains.
This table stores type information for domains, columns, and parameters.
Columns and parameters may have overriding or more specific type information in their specific system tables.

The column `RDB$FIELDS.RDB$FIELD_NAME` is referenced by

* `RDB$FIELD_NAME` in <<fblangref-appx04-fielddims>>
* `RDB$FIELD_SOURCE` in <<fblangref-appx04-funcargs>>
* `RDB$FIELD_SOURCE` in <<fblangref-appx04-procparams>>
* `RDB$FIELD_SOURCE` in <<fblangref-appx04-relfields>>

[#langref-appx04-tbl-fields]
[cols="<4m,<3m,<5a", frame="all", options="header",stripes="none"]
|===
^| Column Name
^| Data Type
^| Description
...
|RDB$FIELD_NAME
|CHAR(63)
|Unique name of a named or system-generated domain.
System-generated domain names start with "`RDB$`".
...
|RDB$VALIDATION_BLR
|BLOB BLR
|Binary language representation (BLR) of the domain `CHECK` constraint condition

|RDB$VALIDATION_SOURCE
|BLOB TEXT
|Source text of the domain `CHECK` constraint condition

|RDB$COMPUTED_BLR
|BLOB BLR
|Binary language representation (BLR) of the computed column expression

|RDB$COMPUTED_SOURCE
|BLOB TEXT
|Source text of a computed column expression

|RDB$DEFAULT_VALUE
|BLOB BLR
|Binary language representation (BLR) of the default value

|RDB$DEFAULT_SOURCE
|BLOB TEXT
|Source text of the default value of the domain
...
`DOUBLE PRECISION`, `BIGINT`, `TIMESTAMP`, `TIME WITH TIME ZONE`, `DECFLOAT(16)`, and `BLOB` occupy 8 bytes.
`TIMESTAMP WITH TIME ZONE` occupies 12 bytes.
`INT128` and `DECFLOAT(34)` occupy 16 bytes.
For `CHAR` and `VARCHAR`, the maximum number of bytes.

|RDB$FIELD_SCALE
|SMALLINT
|Scale of the domain for `DECIMAL` and `NUMERIC`.
For example for `NUMERIC(p,s)`, the value is `-1 * s`, where `s` is the number of digits after the decimal.
...
See also `RDB$FIELD_SUB_TYPE`.

|RDB$FIELD_SUB_TYPE
|SMALLINT
|Subtype of the datatype

For `BLOB`:

[horizontal.compact]
`0`:: `BINARY` -- untyped (binary)
`1`:: `TEXT`
`2`:: `BLR`
`3`:: `ACL`
`4`:: `RANGES`
`5`:: `SUMMARY`
`6`:: `FORMAT` -- encoded table metadata description
`7`:: `TRANSACTION_DESCRIPTION`
`8`:: `EXTERNAL_FILE_DESCRIPTION`
`9`:: `DEBUG_INFORMATION`
`>9`:: reserved for internal or future use by Firebird
`<0`:: user-defined subtype

For integer data types (`SMALLINT`, `INTEGER`, `BIGINT`, `INT128`), and -- dialect 1 -- `DOUBLE PRECISION`:

[horizontal.compact]
`0`:: (or `NULL`) untyped -- data type is specified by `RDB$FIELD_TYPE`. +
If `RDB$FIELD_SCALE` is non-zero, handle as `NUMERIC`.
`1`:: `NUMERIC`
`2`:: `DECIMAL`
...
`0`:: (or `NULL`) untyped -- data type is specified by `RDB$FIELD_TYPE`
`1`:: `BINARY`/`VARBINARY`
`>1`:: untyped -- data type is specified by `RDB$FIELD_TYPE`.
+
Internal use for system domains, related to the character set.
...
|RDB$DESCRIPTION
|BLOB TEXT
|Domain description (`COMMENT ON` text)
...
|RDB$SEGMENT_LENGTH
|SMALLINT
|Suggested segment length (in bytes) for `BLOB`.
`NULL` for all other data types.

Deprecated, and primarily intended for ESQL.
...
|RDB$EXTERNAL_LENGTH
|SMALLINT
|Length in bytes, if it belongs to an external table.
`NULL` for regular tables.

|RDB$EXTERNAL_SCALE
|SMALLINT
|Scale, if it belongs to an external table.

See also `RDB$FIELD_SCALE`.

|RDB$EXTERNAL_TYPE
|SMALLINT
|Data type of the field as represented in an external table.

See also `RDB$FIELD_TYPE`.
...
|SMALLINT
|Length of `CHAR` or `VARCHAR` in characters, of `BINARY` or `VARBINARY` in bytes

|RDB$COLLATION_ID
|SMALLINT
|Identifier of the collation sequence of a character type.

A value of `0` means that no specific collation was specified.

|RDB$CHARACTER_SET_ID
|SMALLINT
|Identifier of the character set of a character or `BLOB TEXT` type

|RDB$FIELD_PRECISION
|SMALLINT
|Precision of an exact numeric data type (_p_ in `DECIMAL(p[,s])`, `NUMERIC(p[,s])`, and `DECFLOAT(p)`.
`0` for integer data types, `NULL` for other data types.

|RDB$SECURITY_CLASS
|CHAR(63)
|Reference to <<fblangref-appx04-secclasses>>

|RDB$OWNER_NAME
|CHAR(63)
|Username of the owner (creator of the domain)
|===
...
`RDB$FILES` stores information about shadow files.

[#langref-appx04-tbl-files]
[cols="<4m,<3m,<5", frame="all", options="header",stripes="none"]
|===
^| Column Name
^| Data Type
...
|RDB$FILE_NAME
|VARCHAR(255)
|The full path to the shadow file
...
|RDB$NULL_FLAG
|SMALLINT
a|Nullable state of the parameter:
...
|SMALLINT
a|Indicates how the type of the parameter was declared

[horizontal.compact]
`0`:: normal (direct type use)
`1`:: declared with `TYPE OF` or `TYPE OF COLUMN`

May be `NULL` for legacy UDFs.
...
The start value is the first value generated by <<langref-commons-nxtvlufor,`NEXT VALUE FOR`>> after a restart of the sequence.

|RDB$GENERATOR_INCREMENT
|INTEGER
|Stores the increment (`INCREMENT BY` value) of the sequence.
The increment is used by <<langref-commons-nxtvlufor,`NEXT VALUE FOR`>>, but not by <<langref-scalarfuncs-gen-id,`GEN_ID`>>.
|===
...
[#langref-appx04-tbl-idxsegments]
[cols="<4m,<3m,<5", frame="all", options="header",stripes="none"]
|===
^| Column Name
^| Data Type
...
`NULL` if this index is not used by a foreign key.
...
[NOTE]
====
In Firebird 6.0 there is a single (pre-defined) publication named `RDB$DEFAULT`.
User-defined publications will be available in a future Firebird release.
====

[#langref-appx04-publication-tables]
== `RDB$PUBLICATION_TABLES`

`RDB$PUBLICATION_TABLES` stores the names of tables that are replicated as part of a publication.

[#langref-appx04-tbl-pubtables]
[cols="<4m,<3m,<5", frame="all", options="header",stripes="none"]
|===
^| Column Name
^| Data Type
...
|RDB$NULL_FLAG
|SMALLINT
a|Nullable state of the column:
...
For example, for the `RDB$OBJECT_TYPE` enumerated type, examples of object types are:
