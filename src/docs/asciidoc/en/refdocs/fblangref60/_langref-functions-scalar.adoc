...
`CLIENT_OS_USER`::
OS username reported by the client.
...
`PAGES_ALLOCATED`::
Number of allocated pages in the database.

`PAGES_FREE`::
Number of free pages in the database.

`PAGES_USED`::
Number of used pages in the database.
...
.See also
<<langref-scalarfuncs-reset-context>>, <<langref-scalarfuncs-set-context>>

[#langref-scalarfuncs-reset-context]
=== `RDB$RESET_CONTEXT()`

Removes all context variables in a given namespace.
...
[[langref-funcs-tbl-rdbresetcontext]]
.`RDB$RESET_CONTEXT` Function Parameters
[cols="<1,<3", options="header",stripes="none"]
|===
^| Parameter
^| Description
...
The `RDB$RESET_CONTEXT` resets the specified context, removing all its context variables.
It returns the number of context variables removed.

Attempts to reset the `SYSTEM` namespace or a non-existent namespace raises an error.

.Example
[source]
----
select RDB$RESET_CONTEXT('USER_SESSION`)
from RDB$DATABASE
----

.See also
<<langref-scalarfuncs-get-context>>, <<langref-scalarfuncs-set-context>>
...
* <<langref-scalarfuncs-reset-context>> can be used to clear all variables from a context
* <<langref-management-session-reset-alter,`ALTER SESSION RESET`>> clears both `USER_TRANSACTION` and `USER_SESSION` contexts.
====
...
.See also
<<langref-scalarfuncs-get-context>>, <<langref-scalarfuncs-reset-context>>
...
[TIP]
====
Use <<langref-aggfuncs-listagg>> or `BLOB_APPEND` functions to concatenate blobs.
...
.See also
<<langref-commons-concat,Concatenation Operator>>, <<langref-aggfuncs-listagg>>, <<langref-sys-pckg-blobutil,`RDB$BLOB_UTIL`>>

[#langref-scalarfuncs-btrim]
=== `BTRIM()`

Multi-character trim "`both`" (leading and trailing)
...
[#langref-funcs-tbl-btrim]
.`BTRIM` Function Parameters
[cols="<1,<3", options="header",stripes="none"]
|===
^| Parameter
^| Description
...
|what
|Characters that are to be trimmed from both sides of the input string _str_.
By default, it is space (`' '`)
|===

`BTRIM` trims _any_ of the character in _what_ from both sides of _str_, whereas <<langref-scalarfuncs-trim>> will only remove leading and/or trailing occurrences of the entire _what_ string.

If _str_ is a `BLOB`, the result is a `BLOB`.
Otherwise, it is a `VARCHAR(__n__)` with _n_ the formal length of _str_.

`BTRIM(_str_)` is equivalent to `RTRIM(LTRIM(_str_))` +
`BTRIM(_str_, _what_)` is equivalent to `RTRIM(LTRIM(_str_, _what_), _what_)`

[#langref-scalarfuncs-btrim-exmpl]
==== `BTRIM` Examples

[source]
----
select btrim('bacbXYZccbba', 'abc') from RDB$DATABASE;
-- XYZ
----

.See also
<<langref-scalarfuncs-ltrim>>, <<langref-scalarfuncs-rtrim>>, <<langref-scalarfuncs-trim>>
...
[#langref-scalarfuncs-ltrim]
=== `LTRIM()`

Multi-character trim leading (left)
...
[#langref-funcs-tbl-ltrim]
.`LTRIM` Function Parameters
[cols="<1,<3", options="header",stripes="none"]
|===
^| Parameter
^| Description
...
|what
|Characters that are to be trimmed from the leading (left) side of the input string _str_.
By default, it is space (`' '`)
|===

`LTRIM` trims _any_ of the character in _what_ from the leading (left) side of _str_, whereas <<langref-scalarfuncs-trim>> will only remove leading and/or trailing occurrences of the entire _what_ string.

If _str_ is a `BLOB`, the result is a `BLOB`.
Otherwise, it is a `VARCHAR(__n__)` with _n_ the formal length of _str_.

[#langref-scalarfuncs-ltrim-exmpl]
==== `LTRIM` Examples

[source]
----
select ltrim('bacbXYZccbba', 'abc') from RDB$DATABASE;
-- XYZccbba
----

.See also
<<langref-scalarfuncs-btrim>>, <<langref-scalarfuncs-rtrim>>, <<langref-scalarfuncs-trim>>
...
[#langref-scalarfuncs-rtrim]
=== `RTRIM()`

Multi-character trim trailing (right)
...
[#langref-funcs-tbl-rtrim]
.`RTRIM` Function Parameters
[cols="<1,<3", options="header",stripes="none"]
|===
^| Parameter
^| Description
...
|what
|Characters that are to be trimmed from the trailing (right) side of the input string _str_.
By default, it is space (`' '`)
|===

`RTRIM` trims _any_ of the character in _what_ from the trailing (right) side of _str_, whereas <<langref-scalarfuncs-trim>> will only remove leading and/or trailing occurrences of the entire _what_ string.

If _str_ is a `BLOB`, the result is a `BLOB`.
Otherwise, it is a `VARCHAR(__n__)` with _n_ the formal length of _str_.

[#langref-scalarfuncs-rtrim-exmpl]
==== `RTRIM` Examples

[source]
----
select rtrim('bacbXYZccbba', 'abc') from RDB$DATABASE;
-- bacbXYZ
----

.See also
<<langref-scalarfuncs-btrim>>, <<langref-scalarfuncs-ltrim>>, <<langref-scalarfuncs-trim>>
...
`TRIM` will remove occurrences of the entire _what_ string from leading and/or trailing sides of _str_, whereas <<langref-scalarfuncs-btrim>>, <<langref-scalarfuncs-ltrim>>, and <<langref-scalarfuncs-rtrim>> will remove occurrences of _any_ character in _what_.
...
.See also
<<langref-scalarfuncs-btrim>>, <<langref-scalarfuncs-ltrim>>, <<langref-scalarfuncs-rtrim>>
...
[#langref-scalarfuncs-cast-format-tbl-fields]
.Cast Template Fields
[cols="<1m,<4a", options="header",stripes="none"]
|===
|Format Pattern
|Description

|YEAR
|Full text year (e.g. "`TWENTY TWENTY-SIX`" for 2026) +
...
|RR / RRRR
a|Round Year (further information <<langref-scalarfuncs-cast-format-rr,below>>) +
...
Exercise caution when combining with year (`YYYY`, etc.).
...
[#langref-scalarfuncs-cast-format-tbl-delim]
...
SELECT CAST(CURRENT_TIMESTAMP AS VARCHAR(45)
  FORMAT 'DD.MM.YYYY HH24:MI:SS "is" J "Julian day"')
FROM RDB$DATABASE;
=========================
...
+
This results in an error if the date is not valid.
For example, `cast('31' as date format 'DD')` works in months with 31 days, but produces error "`__conversion error from string "31"__`" in February, or months with only 30 days.
...
* [[langref-scalarfuncs-cast-format-rr]]Behaviour of `RR` (two-digit Round Year):
...
SELECT CAST('2000.12.08 12:35:30.5000' AS TIMESTAMP
  FORMAT 'YYYY.MM.DD HH24:MI:SS.FF4')
FROM RDB$DATABASE;
=====================
2000-12-08 12:35:30.5000
----

In datetime to string conversions, most template variables produce a fixed length (e.g. `HH24` produces two digits).
However, in string to datetime conversions, those template variable are lenient and will also accept shorter values.
For example, `HH24:MI` produces a string like `01:05`, but will consume both ``'01:05'`` and ``'1:5'`` (etc.) as the same value.

The `FF__n__` format strings produce a value of _n_ digits, but will also consume shorter values.
A shorter value is parsed as if the missing digits are trailing zeroes.
For example, conversion with `HH24:MI:SS.FF4` for strings ``'23:12:05.4000'`` and ``'23:12:05.4'`` produce the same value.

For patterns without delimiters, this behaviour may introduce ambiguity in parsing values and limit roundtrip behaviour.
// TODO Is there actual ambiguity, or is that prevented by the greediness of parsing?
...
.See also
<<langref-scalarfuncs-bin-or>>, <<langref-scalarfuncs-bin-xor>>, <<langref-aggfuncs-bin-and-agg>>
...
.See also
<<langref-scalarfuncs-bin-and>>, <<langref-scalarfuncs-bin-xor>>, <<langref-aggfuncs-bin-or-agg>>
...
.See also
<<langref-scalarfuncs-bin-and>>, <<langref-scalarfuncs-bin-or>>, <<langref-aggfuncs-bin-xor-agg>>
...
Generates a version 4 or version 7 random UUID
...
[#langref-funcs-tbl-gen-uuid]
.`GEN_UUID` Function Parameter
[cols="<1,<3", options="header",stripes="none"]
|===
^| Parameter
^| Description
...
|version
|The UUID version.
Only `4` and `7` are accepted.
The default is `4`, if not specified.
|===

Returns a version 4 or version 7 universally unique ID (UUID) as a 16-byte binary string.
The string format (<<langref-scalarfuncs-uuid-to-char>>) of a version 4 or 7 UUID is `XXXXXXXX-XXXX-YXXX-ZXXX-XXXXXXXXXXXX`, where `Y` is the version (`4` or `7`)
and `Z` is `8`, `9`, `A` or `B`.
...
-- returns e.g. '98044252-7012-4960-9152-C5972528A2BB'
----
