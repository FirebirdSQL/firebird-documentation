...
* Declare local variables, return values and parameters in <<langref-psql,PSQL modules>>, UDRs -- user-defined routines, and UDFs -- user-defined functions
* Provide the type for <<langref-scalarfuncs-cast>> or the `RETURNING` clause of <<langref-tblvalfunc-unlist>> when explicitly converting data from one type to another

[#langref-dtyp-tbl-dtypinfo]
.Overview of Data Types
[cols="<1m,<1,<1,<2a", options="header",stripes="none"]
|===
^| Name
^| Size
^| Precision & Limits
...
``BLOB``s can exceed 4 GB, but some built-in functions and features cannot access data beyond 4 GB.

|<<langref-datatypes-boolean,BOOLEAN>>
|1 byte
|false, true, unknown
|Boolean data type

|<<langref-datatypes-chartypes-char,CHAR{zwsp}[(__n__)]>>, +
CHARACTER{zwsp}[(__n__)]
|_n_ characters
|from 1 to 32,767 bytes
|A fixed-length character data type. +
If length _n_ is not specified, 1 is used by default.

Maximum of length _n_ depends on the character set (e.g. 32,767 for `WIN1252`, 8,191 for `UTF8`).

Values are padded to the declared length with spaces (0x20) -- or NUL (0x00) for character set OCTETS.

|<<langref-datatypes-date,DATE>>
|4 bytes
|From 0001-01-01 AD to 9999-12-31 AD
|Date only, no time element

|<<langref-datatypes-decfloat,DECFLOAT{zwsp}[(_dec_prec_)]>>
|64 or 128 bits
|_dec_prec_ = 16 or 34 (number of decimal digits)
|Decimal floating-point type, IEEE-754 _decimal64_ or _decimal128_. +
If precision _dec_prec_ is not specified, 34 is used by default.

|<<langref-datatypes-decimal,DECIMAL{zwsp}[(__p__[,__s__])]>>, +
DEC{zwsp}[(__p__[,__s__])]
|Varying (32, 64 or 128 bits)
|_p_ = from 1 to 38 +
_s_ = from 0 to _p_
|A number with _p_ digits, with _s_ digits after the decimal point. +
If precision _p_ is not specified, 9 is used by default. +
If scale _s_ is not specified, 0 is used by default. +
_s_ must be less than or equal to _p_.

Example: `DECIMAL(5,3)` contains a number in the following format: `pp.sss`

|<<langref-datatypes-double,DOUBLE PRECISION>>
|64 bits
|2.225 * 10^-308^ to 1.797 * 10^308^
|Double-precision, IEEE-754 _binary64_, ~15 digits, reliable size depends on the platform

|<<langref-datatypes-float,FLOAT>>
|32 bits
|1.175 * 10^-38^ to 3.402 * 10^38^
|Single-precision, IEEE-754 _binary32_, ~7 digits

|<<langref-datatypes-float,FLOAT{zwsp}(_bin_prec_)>>
|32 or 64 bits
...
|<<langref-datatypes-int,INTEGER>>, +
INT
|32 bits
|-2,147,483,648 up to 2,147,483,647
|Signed 32-bit integer.

|<<langref-datatypes-int128,INT128>>
|128 bits
|From -2^127^ to (2^127^ - 1)
|Signed 128-bit integer

|<<langref-datatypes-numeric,NUMERIC{zwsp}[(__p__[,__s__])]>>
...
|<<langref-datatypes-real,REAL>>
|32 bits
|{nbsp}
|Synonym for `FLOAT`

|<<langref-datatypes-smallint,SMALLINT>>
|16 bits
|-32,768 to 32,767
|Signed 16-bit integer

|<<langref-datatypes-time,TIME [WITHOUT TIME ZONE]>>
|4 bytes
|0:00 to 23:59:59.9999
|Time of day.

|<<langref-datatypes-time,TIME WITH TIME ZONE>>
|6 bytes
|0:00 to 23:59:59.9999
|Time of day with either a time zone offset or named zone.

|<<langref-datatypes-timestamp,TIMESTAMP [WITHOUT TIME ZONE]>>
|8 bytes
|From start of day 0001-01-01 AD to end of day 9999-12-31 AD
|Date and time of day

|<<langref-datatypes-timestamp,TIMESTAMP WITH TIME ZONE>>
|10 bytes
|From start of day 0001-01-01 AD to end of day 9999-12-31 AD
|Date and time of day with either a time zone offset or named zone.

|<<langref-datatypes-chartypes-varbinary,VARBINARY{zwsp}[(__n__)]>>, +
BINARY VARYING{zwsp}[(__n__)]
|_n_ bytes
|from 1 to 32,765 bytes
|Variable length string type;
synonym for `VARCHAR(__n__) CHARACTER SET OCTETS`. +
If length _n_ is not specified, 255 is used by default.

Trailing NUL (0x00) are not trimmed, except past the declared length.

|<<langref-datatypes-chartypes-varchar,VARCHAR{zwsp}[(__n__)]>>, +
CHAR VARYING{zwsp}[(__n__)], +
CHARACTER VARYING{zwsp}[(__n__)]
|_n_ characters
|from 1 to 32,765 bytes
|Variable length string type. +
If length _n_ is not specified, 255 is used by default.

Maximum of length _n_ depends on the character set (e.g. 32,765 for `WIN1252`, 8,191 for `UTF8`).
...
Numbers of the `SMALLINT` type are within the range from -2^15^ to 2^15^ - 1, that is, from -32,768 to 32,767.
...
Numbers of the `INTEGER` type are within the range from -2^31^ to 2^31^ - 1, that is, from -2,147,483,648 to 2,147,483,647.
...
Firebird supports two types of floating-point data types:

. <<langref-datatypes-floattypes-approx,Approximate or binary floating-point data types>> (`FLOAT` and `DOUBLE PRECISION`)
. <<langref-datatypes-floattypes-dec,Decimal floating-point types>> (`DECFLOAT`)
...
Precision is dynamic, corresponding to the physical storage format of the value, which is 4 bytes for the `FLOAT` type and 8 bytes for `DOUBLE PRECISION`.

Considering the peculiarities of floating-point numbers, these data types are not recommended for storing monetary data.
For the same reasons, columns of a floating-point type are not recommended for use as primary or unique keys.

For testing data in columns with floating-point data types, expressions should check using a range, e.g. using `BETWEEN`, rather than searching for exact matches.
...
|bin_prec
a|Precision in binary digits, default is 24 (32-bit single precision)
...
* 1 ++<=++ _bin_prec_ ++<=++ 23: 32-bit single precision (synonym for `FLOAT`)
* 25 ++<=++ _bin_prec_ ++<=++ 53: 64-bit double precision (synonym for `DOUBLE PRECISION`)
...
The `DOUBLE PRECISION` data type is stored with an approximate precision of 15 decimal digits (53 binary digits).
...
`DECFLOAT` is a SQL standard numeric type that stores a floating-point number precisely, unlike `FLOAT` or `DOUBLE PRECISION` that provide a binary approximation of the purported precision.
...
A `TIMESTAMP WITH TIME ZONE` has an additional 16-bit integer for the time zone information.
...
If no character set is explicitly specified when defining a character object, the default character set of its containing schema, or -- if not set on the schema -- the database -- at time of defining the object -- will be used.
If neither the schema nor the database have a default character set, the object gets the character set `NONE`.
...
`UNICODE_FSS` is more limited and was previously used by Firebird 3.0 and earlier internally for storing metadata.
Keep in mind that one `UTF8` character occupies up to 4 bytes, thus limiting the size of `CHAR` and `VARCHAR` fields to 8,191 characters (32,767/4 and 32,765/4).

The actual "`bytes per character`" value depends on the range the character belongs to.
Non-accented Latin letters (as defined in `ASCII`) occupy 1 byte, accented characters from Latin-1 (`ISO8859_1`) and Cyrillic letters (`WIN1251`) occupy 2 bytes in `UTF8`.
Characters from other scripts may occupy up to 4 bytes.
...
Each character set has a default collation that specifies the collation order (or, collation sequence, or collating sequence).
Usually, it provides nothing more than ordering based on the numeric code of the characters and a basic mapping of uppercase and lowercase characters.
...
The following table shows the default collations for the `UTF8` character set.
...
Above collations are generic, and not specific to a locale (e.g. language and/or country).
It's possible to derive locale-specific collations using <<langref-ddl-collation-create>>.
...
With case-insensitive collations ("`++_CI++`"), one character in the index key will occupy not 4, but 6 bytes, so the maximum key length for a page size of 8,192 bytes, is 339 characters.
...
.Compatibility Note
Some tools may report the type as `CHAR(__length__) CHARACTER SET OCTETS` instead of `BINARY(__length__)`.
...
|charset
|Character set.
Formally, character set can be qualified with a schema.
That schema is always `SYSTEM`.

|col_schema
|Optional schema of the collation.
...
.Compatibility Notes
Some tools may report the type as `VARCHAR(__length__) CHARACTER SET OCTETS` instead of `VARBINARY(__length__)`.
...
[listing,subs=+quotes]
----
{NCHAR | NATIONAL {CHAR | CHARACTER}} [VARYING] [(_length_)]
  [COLLATE [_col-schema_ <period>] _collation_name_]
----

`NCHAR` is a character data type with the `ISO8859_1` character set.
In all other respects it is the same as `CHAR`, or -- for `VARYING` -- `VARCHAR`.
...
For example, `field1 IS FALSE`, `field1 IS NOT TRUE`, or `field1 IS UNKNOWN`.
...
The segment size for BLOB data is determined by the client, and is usually larger than the data page size.
****
...
A `COLLATE` clause is also supported in some statements and expressions, but contrary to scalar character types, it's not part of the data type declaration syntax.
+
...
Custom subtypes::
Negative subtype numbers can be used for "`user-defined`" subtypes.
Positive subtype numbers are reserved for use by Firebird.
+
Optionally, you can add a name for such custom subtypes.
Custom subtype names can be inserted into the `RDB$TYPES` table by users with the system privilege `CREATE_USER_TYPES`.
+
.Define a subtype `GZIP` with subtype number `-21`
[source]
----
insert into SYSTEM.RDB$TYPES
  (RDB$FIELD_SUB_TYPE, RDB$TYPE, RDB$TYPE_NAME,
   RDB$SYSTEM_FLAG, RDB$DESCRIPTION)
values
  ('RDB$FIELD_SUB_TYPE', -21, 'GZIP', 0, 'GZIP compressed data');
----
+
Both the subtype name and the subtype number can be used: `BLOB SUB_TYPE GZIP` or `BLOB SUB_TYPE -21`.
...
Text BLOBs of any length and any character set can be operands for practically any statement or internal functions.
...
Arrays in Firebird are stored in a `BLOB` of a specialized type.
Arrays can be one-dimensional and multidimensional, and of any data type except `BLOB` or array.
...
.Statement with client-side (emulated) named parameters
[source]
----
select
  SH.SIZE, SH.COLOUR, SH.PRICE
from SHIRTS SH
where (SH.SIZE = :SIZE or :SIZE is null)
  and (SH.COLOUR = :COLOUR or :COLOUR is null)
----

.Actual statement with positional parameters
[source]
----
select
  SH.SIZE, SH.COLOUR, SH.PRICE
...
Explaining what happens here assumes the reader is familiar with the Firebird (legacy) API and the passing of parameters in XSQLVAR structures.
This is probably not of interest to those who are not writing drivers or applications that communicate using the "`naked`" API.

The application passes the parameterized query to the server in the usual positional form (with `?`).
...
[TIP]
====
Since Firebird 6.0, this can also be addressed with the named parameter support provided by the `USING` statement.
// TODO Add link when USING has been documented.

[source]
----
using (SIZE varchar(5) => ?, COLOUR varchar(20) => ?)
select
  SH.SIZE, SH.COLOUR, SH.PRICE
from SHIRTS SH
where (SH.SIZE = :SIZE or :SIZE is null)
...
For the full syntax see <<langref-scalarfuncs-cast,`CAST()`>> in Chapter _Built-in Scalar Functions_.

// TODO Move (or remove if already covered) most of the info below to the CAST description in scalar functions

[#langref-datatypes-convert-domain]
==== Casting to a Domain

When you cast to a domain, its constraints -- `NOT NULL` or `CHECK` -- are taken into account.
If the _value_ does not pass the constraints, the cast will fail.

If `TYPE OF _domain_` is specified, the cast is to its base type, and the domain constraints are ignored.
If this base type is a character type (`CHAR`/`VARCHAR`), the character set and collation are retained.

[#langref-datatypes-convert-typeof]
==== Casting to `TYPE OF COLUMN`

When operands are cast using `TYPE OF COLUMN <column-reference>`, the specified column may be from a table or a view.

Only the type of the column itself is used.
// TODO Is this still accurate with regard to collation?
For character types, this includes the character set, but not the collation.
...
To cast string data types to the `DATE`, `TIME` or `TIMESTAMP` data types, you need the string argument to be one of:

. The predefined datetime mnemonics (see xref:langref-dtyp-tbl-datetimemnemonics[xrefstyle="short"]
. A representation of the date in one of the built-in datetime formats (see <<langref-commons-datetime-format-syntax,Datetime Format Syntax>>)
. A custom datetime format using the <<langref-scalarfuncs-cast-format,`FORMAT` clause>>
...
select
  ...
  -- custom format
  cast('20230719 161757' as timestamp format 'YYYYMMDD HH24MISS')
from RDB$DATABASE
----
...
|`NOT NULL`
|No
|Often it is better to leave a domain nullable in its definition and decide whether to make it `NOT NULL` when using the domain to define columns.
|===
...
<blob_datatype> ::=
  !! See <<langref-datatypes-syntax-blob-syntax,Blob Data Types Syntax>> !!
----
...
[#langref-datatypes-syntax-scalar-collate]
==== Use of Collations in Declarations

The `COLLATE` clause can be used to specify the <<langref-datatypes-chartypes-collation,collation>> to apply for a use of a character type.
A collation is used for sorting and comparisons.

A collation can be a <<langref-appx08-charsets,built-in collation>>, stored in the `SYSTEM` schema, or a <<langref-ddl-collation,user-defined collation>> in some other schema.

Since Firebird 6.0, the `COLLATE` clause is part of the data type declaration syntax of character types (but not for text BLOBs nor character type arrays).
In previous versions, the `COLLATE` clause was a separate statement or expression specific clause, where the position could vary, and sometimes was absent for some data type declarations.
For example, in a column definition in `CREATE TABLE`, the `COLLATE` clause would come only after the `DEFAULT` clause and constraints.
For backward compatibility, such alternative placement is still supported, as long as the `COLLATE` clause is specified only once.
