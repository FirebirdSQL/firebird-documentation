...
See <<create-procedure-examples,an example of an executable stored procedure>> at the end of the <<langref-ddl-proc-create,`CREATE PROCEDURE` section>> of xref:fblangref50-ddl[xrefstyle=full].
...
See <<create-procedure-examples,an example of a selectable stored procedure>> at the end of the <<langref-ddl-proc-create,`CREATE PROCEDURE` section>> of xref:fblangref50-ddl[xrefstyle=full].
...
For information about creating stored procedures, see <<langref-ddl-proc-create,`CREATE PROCEDURE`>> in xref:fblangref50-ddl[xrefstyle=full].
...
For information about creating stored functions, see <<langref-ddl-func-create,`CREATE FUNCTION`>> in xref:fblangref50-ddl[xrefstyle=full].
...
For information on creating packages, see <<langref-ddl-pkg-create,`CREATE PACKAGE`>>, and <<langref-ddl-pkg-body-create,`CREATE PACKAGE BODY`>> in xref:fblangref50-ddl[xrefstyle=full].
...
See also <<langref-funcs-tbl-rdbgetcontext-ddl-trigger,The `DDL_TRIGGER` Namespace>> in <<langref-scalarfuncs-get-context,`RDB$GET_CONTEXT`>> in xref:fblangref50-functions[xrefstyle=full].

[#langref-psql-triggercreate]
=== Creating Triggers

For information on creating triggers, see <<langref-ddl-trgr-create,`CREATE TRIGGER`>>, <<langref-ddl-trgr-crtalter,`CREATE OR ALTER TRIGGER`>>, and <<langref-ddl-trgr-recreate,`RECREATE TRIGGER`>> in xref:fblangref50-ddl[xrefstyle=full].
