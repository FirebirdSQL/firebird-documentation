...
[#langref-psql-elements-transacs]
=== Transactions
...
Statements that start and end transactions are not available in PSQL, but it is possible to run a statement or a block of statements in an <<langref-psql-inauton,autonomous transaction>>.

[#langref-psql-elements-structure]
=== Module Structure
...
<declare-var> ::=
  !! See <<langref-psql-declare-variable,DECLARE VARIABLE>> !!

<declare-cursor> ::=
  !! See <<langref-psql-declare-cursor,DECLARE .. CURSOR>> !!

<subfunc-forward>, <subfunc-def> ::=
  !! See <<langref-psql-declfunc,DECLARE FUNCTION>> !!

<subproc-forward>, <subproc-def> ::=
  !! See <<langref-psql-declproc,DECLARE PROCEDURE>> !!
----

[#langref-psql-elements-tbl-body]
...
[#langref-psql-elements-body-psql]
...
[#langref-psql-elements-body-ext]
...
[#langref-psql-storedprocs-benefits]
...
See <<create-procedure-examples,an example of an executable stored procedure>> at the end of the <<langref-ddl-proc-create,`CREATE PROCEDURE` section>> of xref:langref-ddl[xrefstyle=full].

[#langref-psql-storedprocs-selectable]
...
A <<langref-psql-suspend>> statement at the end of the iteration fills the buffer and waits for the caller to fetch the row.
...
See <<create-procedure-examples,an example of a selectable stored procedure>> at the end of the <<langref-ddl-proc-create,`CREATE PROCEDURE` section>> of xref:langref-ddl[xrefstyle=full].

[#langref-psql-storedprocs-creating]
=== Creating a Stored Procedure

The syntax for creating executable stored procedures and selectable stored procedures is the same.
The difference comes in the logic of the program code, specifically the absence or presence of a <<langref-psql-suspend>> statement.

For information about creating stored procedures, see <<langref-ddl-proc-create,`CREATE PROCEDURE`>> in xref:langref-ddl[xrefstyle=full].

[#langref-psql-storedprocs-modifying]
=== Modifying a Stored Procedure

For information about modifying existing stored procedures, see <<langref-ddl-proc-alter,`ALTER PROCEDURE`>>, <<langref-ddl-proc-crtoralter,`CREATE OR ALTER PROCEDURE`>>, <<langref-ddl-proc-recreate,`RECREATE PROCEDURE`>>.

[#langref-psql-storedprocs-deleting]
=== Dropping a Stored Procedure

For information about dropping (deleting) stored procedures, see <<langref-ddl-proc-drop,`DROP PROCEDURE`>>.

[#langref-psql-storedfuncs]
== Stored Functions
...
[#langref-psql-storedfuncs-create]
=== Creating a Stored Function

For information about creating stored functions, see <<langref-ddl-func-create,`CREATE FUNCTION`>> in xref:langref-ddl[xrefstyle=full].

[#langref-psql-storedfuncs-modifying]
=== Modifying a Stored Function

For information about modifying stored functions, see <<langref-ddl-func-alter,`ALTER FUNCTION`>>, <<langref-ddl-func-creatalter,`CREATE OR ALTER FUNCTION`>>, <<langref-ddl-func-recreate,`RECREATE FUNCTION`>>.

[#langref-psql-storedfuncs-deleting]
=== Dropping a Stored Function

For information about dropping (deleting) stored functions, see <<langref-ddl-func-drop,`DROP FUNCTION`>>.

[#langref-psql-dynblocks]
== PSQL Blocks
...
.See also
See <<langref-dml-execblock,`EXECUTE BLOCK`>> for details.

[#langref-psql-package]
== Packages
...
[#langref-psql-pkg-create]
=== Creating a Package

For information on creating packages, see <<langref-ddl-pkg-create,`CREATE PACKAGE`>>, and <<langref-ddl-pkg-body-create,`CREATE PACKAGE BODY`>> in xref:langref-ddl[xrefstyle=full].

[#langref-psql-pkg-alter]
=== Modifying a Package

For information on modifying existing package header or bodies, see <<langref-ddl-pkg-alter,`ALTER PACKAGE`>>, <<langref-ddl-pkg-createalter,`CREATE OR ALTER PACKAGE`>>, <<langref-ddl-pkg-recreate,`RECREATE PACKAGE`>>, and <<langref-ddl-pkg-body-recreate,`RECREATE PACKAGE BODY`>>.

[#langref-psql-pkg-drop]
=== Dropping a Package

For information on dropping (deleting) a package, see <<langref-ddl-pkg-drop,`DROP PACKAGE`>>, and <<langref-ddl-pkg-body-drop,`DROP PACKAGE BODY`>>.

[#langref-psql-triggers]
== Triggers
...
The Boolean context variables <<langref-contextvars-inserting,`INSERTING`>>, <<langref-contextvars-updating,`UPDATING`>> and <<langref-contextvars-deleting,`DELETING`>> can be used in the body of a trigger to determine the type of event that fired the trigger.
...
See also <<langref-funcs-tbl-rdbgetcontext-ddl-trigger,The `DDL_TRIGGER` Namespace>> in <<langref-scalarfuncs-get-context,`RDB$GET_CONTEXT`>> in xref:langref-functions[xrefstyle=full].

[#langref-psql-triggercreate]
=== Creating Triggers

For information on creating triggers, see <<langref-ddl-trgr-create,`CREATE TRIGGER`>>, <<langref-ddl-trgr-crtalter,`CREATE OR ALTER TRIGGER`>>, and <<langref-ddl-trgr-recreate,`RECREATE TRIGGER`>> in xref:langref-ddl[xrefstyle=full].

[#langref-psql-triggermodify]
=== Modifying Triggers

For information on modifying triggers, see <<langref-ddl-trgr-alter,`ALTER TRIGGER`>>, <<langref-ddl-trgr-crtalter,`CREATE OR ALTER TRIGGER`>>, and <<langref-ddl-trgr-recreate,`RECREATE TRIGGER`>>.

[#langref-psql-triggerdelete]
=== Dropping a Trigger

For information on dropping (deleting) triggers, see <<langref-ddl-trgr-drop,`DROP TRIGGER`>>.
...
[#langref-psql-coding-assign-exmpl]
...
.See also
<<langref-psql-declare-variable>>

[#langref-psql-coding-management]
...
[none]
* <<langref-management-session-reset-alter,`ALTER SESSION RESET`>>
* <<langref-management-setbind,`SET BIND`>>
* <<langref-management-setdecfloat,`SET DECFLOAT`>>
* <<langref-management-role-set,`SET ROLE`>>
* <<langref-management-setsessionidle,`SET SESSION IDLE TIMEOUT`>>
* <<langref-management-setstatementtimeout,`SET STATEMENT TIMEOUT`>>
* <<langref-management-settimezone,`SET TIME ZONE`>>
* <<langref-management-role-set-trusted,`SET TRUSTED ROLE`>>

[#langref-psql-coding-management-exmpl]
...
.See also
<<langref-management,Management Statements>>
...
[#langref-psql-tbl-declare-variable]
...
|expr
|Expression for the initial value for this variable.
The expression can reference input parameters, and previously declared variables, subprocedures and subfunctions.
...
A special case of `DECLARE [VARIABLE]` -- declaring cursors -- is covered separately in <<langref-psql-declare-cursor>>

[#langref-psql-variable-type]
...
[#langref-psql-variable-charsetcollate]
...
The `COLLATE` clause is also part of <<langref-datatypes-syntax-scalar,_domain_or_non_array_type_>> for character types, and should be specified only once.

[#langref-psql-variable-default]
...
The initial value can be specified in two ways, `DEFAULT <expr>` and `= <expr>`.
The value can be any type-compatible expression.

The expression can reference input parameters and previously declared variables, subprocedures and subfunctions, except where this results in a circular dependency (e.g. a subfunction is forward-declared, a variable is initialised using that function, and then the subfunction is declared referencing that variable).
When referencing other variables, the value at time of declaration is used.
...
.See also
<<langref-datatypes,Data Types and Subtypes>>, <<langref-datatypes-custom,Custom Data Types -- Domains>>, <<langref-ddl-domn-create,`CREATE DOMAIN`>>

[#langref-psql-declare-cursor]
=== `DECLARE .. CURSOR`
...
[#langref-psql-tbl-declare-cursor]
...
Syntactically, the `DECLARE ... CURSOR` statement is a special case of <<langref-psql-declare-variable>>.

[#langref-psql-declare-cursor-types]
...
Forward-only cursors only support the <<langref-psql-fetch,`FETCH [NEXT FROM]`>> statement, other fetch options raise an error.
...
.See also
<<langref-psql-open>>, <<langref-psql-fetch>>, <<langref-psql-close>>

[#langref-psql-declfunc]
=== `DECLARE FUNCTION`
...
<subfunc-header>  ::=
  ...
  RETURNS <domain_or_non_array_type> [NOT NULL] [COLLATE _collation_]
  [DETERMINISTIC]

<in_params> ::=
  !! See <<langref-ddl-func-create-syntax,`CREATE FUNCTION` Syntax>> !!
...
.See also
<<langref-psql-declproc>>, <<langref-ddl-func-create,`CREATE FUNCTION`>>

[#langref-psql-declproc]
=== `DECLARE PROCEDURE`
...
<in_params> ::=
  !! See <<langref-ddl-proc-create-syntax,`CREATE PROCEDURE` Syntax>> !!
...
.See also
<<langref-psql-declfunc>>, <<langref-ddl-proc-create,`CREATE PROCEDURE`>>

[#langref-psql-beginend]
=== `BEGIN ... END`
...
For <<langref-psql-handleexceptions,error handling>>, you can add one or more <<langref-psql-when>> statements immediately before `END`.
...
.See also
<<langref-psql-exit>>, <<langref-sidebar01,`SET TERM`>>, <<langref-psql-when>>
...
----
+
This specific example can be replaced with a <<langref-commons-conditional-case-simple,simple `CASE`>> or the <<langref-scalarfuncs-decode,`DECODE`>> function.

.See also
<<langref-psql-while>>, <<langref-commons-conditional-case,`CASE`>>
...
.See also
<<langref-psql-ifthen>>, <<langref-psql-break>>, <<langref-psql-leave>>, <<langref-psql-continue>>, <<langref-psql-exit>>, <<langref-psql-forselect>>, <<langref-psql-forexec>>, <<langref-psql-range-for>>
...
<loop_stmt> ::=
  ...
  | FOR _variable_ = <initial_value>
    {TO | DOWNTO} <final_value> [BY <step_value>] DO
  | WHILE (<condition>) DO }
----

[#langref-psql-tbl-break]
...
.See also
<<langref-psql-break>>, <<langref-psql-continue>>, <<langref-psql-exit>>

[#langref-psql-continue]
=== `CONTINUE`
...
<loop_stmt> ::=
  { FOR <select_stmt> INTO <var_list> DO
  | FOR EXECUTE STATEMENT ... INTO <var_list>
  | FOR _variable_ = <initial_value>
    {TO | DOWNTO} <final_value> [BY <step_value>] DO
  | WHILE (<condition>) DO }
----

[#langref-psql-tbl-continue]
...
.See also
<<langref-psql-break>>, <<langref-psql-leave>>, <<langref-psql-exit>>
...
.See also
<<langref-psql-break>>, <<langref-psql-leave>>, <<langref-psql-continue>>, <<langref-psql-suspend>>

[#langref-psql-suspend]
=== `SUSPEND`
...
The `SUSPEND` statement is used in selectable stored procedures or `EXECUTE BLOCK` to pass the values of output parameters to a buffer and suspend execution.
...
[IMPORTANT]
====
. The `SUSPEND` statement can only occur in stored procedures, sub-procedures, or `EXECUTE BLOCK`
...
<connection_string> ::=
  !! See <filespec> in the <<langref-ddl-db-create-syntax,`CREATE DATABASE` syntax>> !!

<variables> ::= [:{endsb}__varname__ [, [:{endsb}__varname__ ...]
----

[#langref-psql-tbl-execstmt]
...
Statements producing multiple rows of data must be executed with <<langref-psql-forexec>>.

[#langref-psql-execstmt-wparams]
...
[#langref-psql-execstmt-wparams01]
...
[#langref-psql-execstmt-wparms-exmpl]
...
[#langref-psql-execstmt-wautonomous]
==== `WITH {AUTONOMOUS | COMMON} TRANSACTION`
...
See <<langref-psql-inauton>> for more details on autonomous transactions.
...
[#langref-psql-execstmt-onexternal]
...
[#langref-psql-execstmt-onext-connpool]
===== Connection Pooling
...
[#langref-psql-execstmt-onext-transpool]
===== Transaction Pooling
...
[#langref-psql-execstmt-onext-errhandling]
===== Exception Handling
...
[#langref-psql-execstmt-onext-morenotes]
===== Miscellaneous Notes
...
.See also
<<langref-psql-forexec>>

[#langref-psql-forselect]
=== `FOR SELECT`
...
// When updating, also update the same list in langref-psql-fetch
...
.See also
<<langref-psql-declare-cursor>>, <<langref-psql-forexec>>, <<langref-psql-range-for>>, <<langref-psql-break>>, <<langref-psql-leave>>, <<langref-psql-continue>>, <<langref-psql-exit>>, <<langref-dml-select,`SELECT`>>, <<langref-dml-update,`UPDATE`>>, <<langref-dml-delete,`DELETE`>>
...
.See also
<<langref-psql-execstmt>>, <<langref-psql-forselect>>, <<langref-psql-break>>, <<langref-psql-leave>>, <<langref-psql-continue>>

[#langref-psql-range-for]
=== Range-based `FOR`

Range-based for, or "`for loop`"
...
The range-based `FOR` statement iterates over a range of numeric values.
...
* <<langref-psql-break>> or <<langref-psql-leave>> exits the loop
* <<langref-psql-continue>> skips the remainder of the loop body and starts the next iteration
* _variable_ can be modified by the loop body

[#langref-psql-range-for-exmpl]
...
.See also
<<langref-psql-break>>, <<langref-psql-leave>>, <<langref-psql-continue>>
...
`OPEN` can be applied only to cursors previously declared in a <<langref-psql-declare-cursor>> statement.
...
.See also
<<langref-psql-declare-cursor>>, <<langref-psql-fetch>>, <<langref-psql-close>>
...
The `FETCH` statement can be used only with a cursor declared with the <<langref-psql-declare-cursor>> statement.
...
// When updating also update the same list in langref-psql-undeclaredcursor
.Rules for Cursor Variables
* When accessing a cursor variable in a DML statement, the colon prefix can be added before the cursor name (i.e. `:__cursor_name__.__columnname__`) for disambiguation, similar to variables.
+
The cursor variable can be referenced without colon prefix, but in that case, depending on the scope of the contexts in the statement, the name may resolve in the statement context instead of to the cursor (e.g. you select from a table with the same name as the cursor).
...
This means that an `UPDATE` statement (with a `WHERE CURRENT OF` clause) will update not only the table, but also the fields in the cursor variable for subsequent reads.
Executing a `DELETE` statement (with a `WHERE CURRENT OF` clause) will set all fields in the cursor variable to `NULL` for subsequent reads
// Don't include in langref-psql-undeclaredcursor
...
.See also
<<langref-psql-declare-cursor>>, <<langref-psql-open>>, <<langref-psql-close>>
...
Only a cursor that was declared with <<langref-psql-declare-cursor>> can be closed with a `CLOSE` statement.
Any cursors that are still open will be automatically closed after the module code completes execution.

[#langref-psql-close-exmpl]
==== `CLOSE` Examples

See <<langref-psql-fetch-exmpl>>

.See also
<<langref-psql-declare-cursor>>, <<langref-psql-open>>, <<langref-psql-fetch>>

[#langref-psql-inauton]
...
.See also
<<langref-transacs,Transaction Control>>

[#langref-psql-postevent]
=== `POST_EVENT`
...
[#langref-psql-return-exmpl]
==== `RETURN` Examples

See <<langref-ddl-func-create-example,`CREATE FUNCTION` Examples>>

[#langref-psql-handleexceptions]
...
In PSQL code, exceptions are handled by means of the <<langref-psql-when>> statement.
...
Complete listings of the system exceptions can be found in xref:langref-appx02-errorcodes[xrefstyle=full]:
...
For details, see <<langref-ddl-exception-create,`CREATE EXCEPTION`>>.

[#langref-psql-exception]
=== `EXCEPTION`
...
Exceptions can be handled in a <<langref-psql-when>> statement.
...
.See also
<<langref-ddl-exception-create,`CREATE EXCEPTION`>>, <<langref-psql-when>>
...
Use the <<langref-scalarfuncs-rdberror,`RDB$ERROR`>> function to obtain the SQLCODE, GDSCODE, SQLSTATE, custom exception name and exception message.
The `EXCEPTION` statement, without parameters, can also be used in this context to re-throw the error or exception.

[#langref-psql-when-gdscode]
.Targeting `GDSCODE`
****
...
Use of <<langref-ddl-tbl-external,external tables>> can be useful here, as data written to them is transaction-independent.
...
.See also
<<langref-psql-exception>>, <<langref-ddl-exception-create,`CREATE EXCEPTION`>>, <<langref-appx02-tbl-errcodes01,SQLCODE and GDSCODE Error Codes and Message Texts>> and <<langref-appx02-tbl-sqlstates,SQLSTATE Codes and Message Texts>>, <<langref-contextvars-gdscode,`GDSCODE`>>, <<langref-contextvars-sqlcode,`SQLCODE`>>, <<langref-contextvars-sqlstate,`SQLSTATE`>>, <<langref-scalarfuncs-rdberror,`RDB$ERROR()`>>
