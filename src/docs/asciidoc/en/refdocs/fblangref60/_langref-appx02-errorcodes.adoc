...
For more information, see <<langref-ddl-exception-create,`CREATE EXCEPTION`>> in xref:fblangref50-ddl[] and, for usage, the statement <<langref-psql-exception,`EXCEPTION`>> in xref:fblangref50-psql[].
====
