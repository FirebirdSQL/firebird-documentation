...
|default_charset
|Specifies the default character set for character data types created inside this schema.
...
If not specified, the default character set of the database is inherited.
The default will be used for objects created in the schema except where an alternative character set is used explicitly for a field, domain, variable, cast expression, etc.
+
...
If not specified, the `SQL SECURITY` option of the database is inherited.
...
|default-charset
|The default character set for character data types created inside this schema.
The character set can optionally be qualified with a schema;
only `SYSTEM` has character sets.
|===
