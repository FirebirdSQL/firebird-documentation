...
CREATE FUNCTION [IF NOT EXISTS]
  ...
  RETURNS <domain_or_non_array_type> [NOT NULL]
...
It is also possible to explicitly specify `NOT DETERMINISTIC`;
...
* code in the body of the stored function
* mark a function as deterministic or non-deterministic
* define, change, or drop its SQL Security property
...
Altering a functions body -- but not individual function options -- without specifying the `SQL SECURITY` clause will remove the SQL Security property if currently set for this function.
...
.Altering only deterministic option of a function
[source]
----
-- Mark as deterministic
alter function ADD_INT deterministic;
-- Mark as non-deterministic
alter function ADD_INT not deterministic;
----

.Altering only SQL security of a function
[source]
----
-- Set to INVOKER
alter function ADD_INT sql security invoker;
-- Set to DEFINER
alter function ADD_INT sql security definer;
-- Drop SQL security (revert to schema default)
alter function ADD_INT drop sql security;
----
