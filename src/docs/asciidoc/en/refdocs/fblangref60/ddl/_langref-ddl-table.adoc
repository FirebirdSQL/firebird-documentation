...
.Syntax
[listing#langref-ddl-tbl-create-syntax,subs="+quotes,macros"]
----
...
<cast-function> ::=
  !! See <<langref-scalarfuncs-cast>> !!

<routine-location> ::=
  !! see <<langref-dml-select-from-syntax,Select `FROM` syntax>> !!

<in-source> ::=
  !! See <<langref-commons-in-syntax,`IN` syntax>> !!
...
// TODO Reduce repetition with langref-ddl-domn-create-syntax
...
The `COLLATE` clause is also part of <<langref-datatypes-syntax-scalar,_scalar_datatype_>> for character types, and should be specified only once.
...
A more in-depth coverage of the row format is available in chapter https://firebirdsql.org/docs/html/en/firebirddocs/firebirdinternals/firebird-internals.html#fbint-external-table[External Table Format] in https://firebirdsql.org/docs/html/en/firebirddocs/firebirdinternals/firebird-internals.html[Firebird Internals: Inside a Firebird Database].
...
=== The `ADD COLUMN` Clause

The `ADD COLUMN` clause defines a new column.

When `IF NOT EXISTS` is specified and the column already exists, it will not raise an error and ignore this table modification.
...
=== The `ADD CONSTRAINT` Clause

The `ADD CONSTRAINT` clause defines a -- optionally named -- table constraint.
...
The `DROP [COLUMN]` clause deletes the specified column from the table.

When `IF EXISTS` is specified and the column does not exist, it will not raise an error and ignore this table modification.

An attempt to drop a column will fail if anything references the column.
