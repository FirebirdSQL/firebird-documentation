...
The `ALTER PACKAGE` statement modifies the package header, or defines, changes, or drops its SQL Security property.
...
Altering a package header without specifying the `SQL SECURITY` clause will remove the SQL Security property if currently set for this package.
