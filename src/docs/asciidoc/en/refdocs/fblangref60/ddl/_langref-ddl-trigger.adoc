...
CREATE TRIGGER [IF NOT EXISTS]
  [_trig-schema_ <period>] _trigname_
...
When `IF NOT EXISTS` is specified, the statement will not raise an error if the trigger already exists.
...
When `IF EXISTS` is specified, the statement will not raise an error if the trigger does not exist.
