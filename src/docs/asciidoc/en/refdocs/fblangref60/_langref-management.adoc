...
The current value for the connection can be found using <<fblangref50-scalarfuncs-get-context,`RDB$GET_CONTEXT('SYSTEM', 'DECFLOAT_ROUND')`>>.
...
The current value for the connection can be found using <<fblangref50-scalarfuncs-get-context,`RDB$GET_CONTEXT('SYSTEM', 'DECFLOAT_TRAPS')`>>.
...
A reset of the unused connection is attempted using an <<fblangref50-management-session-reset-alter>> statement and,
...
The current statement timeout set for the session can be retrieved through <<fblangref50-scalarfuncs-get-context,`RDB$GET_CONTEXT`>>, namespace `SYSTEM` and variable `STATEMENT_TIMEOUT`.
Information is also available from `MON$ATTACHMENTS`:
...
[[fblangref50-management-optimize]]
== Optimizer Configuration

[[fblangref50-management-optimize-set]]
=== `SET OPTIMIZE`
...
It can also be specified at the statement level using the <<fblangref50-dml-selec-optimize,`OPTIMIZE FOR`>> clause.
