...
[horizontal.compact]
DSQL::
_Dynamic SQL_ is the major part of the language which corresponds to Part 2 (SQL/Foundation) of the SQL specification (ISO/IEC 9075-2).
...
PSQL corresponds to Part 4 (SQL/PSM) of the SQL specification (ISO/IEC 9075-4).
...
ISQL::
_Interactive SQL_ refers to the language that can be executed using Firebird _isql_, the command-line application for accessing databases interactively.
...
* _Dialect 1_ (deprecated) is intended solely to allow backward compatibility with legacy databases from old InterBase versions, version 5 and below.
...
** The `TIME` data type stores time-of-day data.
...
** Double quotes are used for delimited identifiers, enabling object names that are case-sensitive or that do not meet the requirements for regular identifiers in other ways.
** Strings are delimited with single quotes (apostrophes) or using <<langref-commons-qstrings,Q-strings>>.
...
Dialect 1 is deprecated and support will be removed in a future Firebird version.
...
----
+
On the contrary, the following statement will return an error because `ADD` is a reserved keyword.
+
[source]
----
...
----
<identifier chain> ::=
  <identifier> [<scope-specifier>] [ { <period> <identifier> }... ]

<scope-specifier> ::= '%' { SCHEMA | PACKAGE }
----

The maximum length of the chain depends on the context and the type of referenced object.

The _scope-specifier_ can be used for disambiguation between package and schema references.
By default, an ambiguous reference will first be tried as a schema, and then as a package in the schemas on the search path.
When the _scope-specifier_ is used, it will only be tried as a schema *or* as a package in the schemas on the search path.

Use of a _scope-specifier_ might not be valid in all syntax contexts where an _identifier chain_ is accepted.
...
Details about literals for each data type are discussed in section <<langref-commons-constants>> of chapter <<langref-commons>>.
...
Firebird supports two types of comments: _block_ (or _bracketed_) and _in-line_ (or _single-line_ or _simple_).
