...
[[rnfb50-config-update-overwrite]]
=== AllowUpdateOverwrite

It is possible that `BEFORE UPDATE` or `AFTER UPDATE` trigger was fired previously by the current `UPDATE` statement for the other record and has already changed the current record being updated. Due to cursor stability, `UPDATE` should not see such changes and thus could silently overwrite them.

This new setting defines how `UPDATE` operation should handle the case when a record was already updated by trigger or by the another positional `UPDATE` in the same cursor:

* if set to true (default), prior changes are overwritten
* if set to false, error "UPDATE will overwrite changes made by the trigger or by the another UPDATE in the same cursor" is raised
