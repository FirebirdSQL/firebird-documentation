...
-- Wrong error message on login if the user doesn't exists and _WireCrypt_ is disabled  
...
-- `RDB$TIME_ZONE_UTIL.TRANSITIONS` returns an infinite resultset  
...
-- Error "attempt to evaluate index expression recursively"  
...
-- Fix performance issues in _prepare_update()_  
...
-- Error during restore: "Index cannot be used in the specified plan"  
