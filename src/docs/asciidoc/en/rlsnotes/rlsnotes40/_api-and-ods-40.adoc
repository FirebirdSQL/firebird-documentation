...
After that, the batch can be executed or the next portion of messages can be added to it.
...
[[rnfb40-apiods-api-util-convert]]
=== `Util::convert()`

A new method, `convert`, was added to the `Util` interface.
This method converts values stored inside a message between different data types in the same way as the engine does internally.
It works for all data types except blobs and arrays.

[source]
----
interface Util
...
	void convert(Status status,
		uint sourceType, uint sourceScale, uint sourceLength, const void* source,
